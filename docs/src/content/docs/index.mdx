...
flowchart LR
  ...
  subgraph after[" With no-mistakes "]
    ...
    G --> H["Push verified commit"]
    H --> I["Supported-host happy path: open PR + watch CI"]
  end
...
A gated push validates and publishes a rough branch before any supported-host PR automation begins.
...
| One model for everything | Every model call routed by purpose through each profile's declared candidates |
| Manually raise and shepherd the PR | With host support, PR opened and CI watched while it remains open |
