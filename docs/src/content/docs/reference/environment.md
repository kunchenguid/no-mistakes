...
They include only flag-derived context: `/axi/run` records whether `--yes`, `--intent`, `--skip`, or `--commit-trailer` was present, and `/axi/respond` records the sanitized action and whether `--yes` was present.
