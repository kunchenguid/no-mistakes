...
## Strict repository context overrides

A repository initialized with [`--github-context`](/no-mistakes/reference/cli/#strict-per-repository-github-context) does not use ambient GitHub account selection. For GitHub and Git children belonging to that repository, no-mistakes removes `GH_TOKEN`, `GITHUB_TOKEN`, `GH_ENTERPRISE_TOKEN`, `GITHUB_ENTERPRISE_TOKEN`, ambient `GH_CONFIG_DIR`, `GH_HOST`, and `GH_REPO`; Git config-injection variables; global/system Git config selection; askpass and SSH selection; and Git author/committer overrides. It then installs only the typed repository context. This is process-local and does not mutate the daemon environment, global `gh` state, or global/system Git configuration, so two repositories can run concurrently with different contexts.

Repositories without the optional context retain the ambient behavior described on this page. Other providers are unaffected.
