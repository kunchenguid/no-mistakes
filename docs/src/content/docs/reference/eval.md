...
The replay never inherits this machine's own harness pins: capture strips `agent`, `review_fix_agent`, `agent_args_override`, and `agent_config` from the configuration it freezes, so the candidate is the only thing that decides what the harness runs as.
