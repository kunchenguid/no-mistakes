...
When unset, no-mistakes behaves exactly as before: branch deltas, rebases, pull-request discovery and creation, PR summaries, and CI base-tip monitoring use the repository's discovered default branch. When set, those operations consistently use `pr.base_branch` instead. This supports release flows where feature PRs land in an integration branch and promotion from that branch to the repository default is handled separately. With an explicit base, new runs are refused on that branch and, when it is distinct, the actual repository default branch; put gated work on a separate feature branch.

The target must be a short branch name such as `quality-assurance`, not a full ref such as `refs/heads/quality-assurance`. Option-like names, whitespace, ref traversal or revision syntax, control characters, and other names Git rejects are invalid. Before a run launches an agent, no-mistakes fetches the configured branch from the upstream repository and requires it to resolve and have shared Git history with the feature branch (a usable merge base). That validated commit becomes the run's immutable base snapshot for branch-delta operations and Rebase, and recovery preserves it. CI separately refreshes and validates the moving upstream target for base-tip monitoring and conflict repair, so a later force-push to unrelated history is refused before repair. A missing, malformed, unsafe, unreachable, or unrelated target fails the run with an error naming `pr.base_branch`; no-mistakes never silently falls back to the default branch for an explicit value.
...
If the trusted setting changes while a branch still has an open pull request against its previous base, a replacement push or rerun preserves that base only after the provider verifies the saved head/base pull request is open. This lets the run update it instead of silently retargeting or creating a duplicate. If there is no such open pull request, the current trusted setting applies; an unavailable provider or unresolved state fails the continuity check rather than guessing.
