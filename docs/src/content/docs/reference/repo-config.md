...
Because the policy is trusted-only, a submitted branch that declares `protected_paths` entries the default branch does not carry is **refused before the run starts** so auto-fix cannot proceed with the requested protection silently dropped. Recovery applies the same check before resuming a run. The refusal names the unsupported entries and points to the default branch. Entries are compared individually by their pattern text after trimming surrounding whitespace, not by whether their globs cover the same files. A branch that omits the setting, or only re-declares entries the default branch already carries, is unaffected; the full trusted policy still applies. To resolve the configuration error, add the same entries to the default branch's `.no-mistakes.yaml`, or remove the unsupported entries from the submitted branch if that protection is no longer needed.
...
A refusal to stage dirty protected paths always requires an explicit response, including under AXI `--yes` and TUI yolo mode. CI does not retry its fixer, and automatic gate reconciliation cannot clear the refusal, even if the PR closes. Approval is rejected because it would skip unfinished work: inspect and resolve the reported edit, then use `no-mistakes axi respond --action fix` to retry the step, including its commit and publication. Deliberate skip and abort behavior is unchanged.
