...
The publication path synchronizes the local gate mirror before atomically recording the repaired head and push binding. If mirror settlement fails after the remote moved, nothing is recorded as published; before consulting or charging the auto-fix budget, the monitor retries the already-created repair, verifies the remote is already at that head, and completes publication once the mirror works. The ordinary Push step follows the same settlement order and fails for a later retry.
