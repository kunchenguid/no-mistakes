...
Because the policy is trusted-only, a submitted branch that declares `protected_paths` entries the default branch does not carry is **refused before the run starts** rather than failing open with its declaration silently dropped (which would let the branch's own auto-fix mutate the very paths it asked to protect). The refusal names the dropped entries and points to the default branch. A branch that omits the setting, or merely re-declares entries the default branch already carries, is unaffected.
