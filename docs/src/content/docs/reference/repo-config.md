...
It is deliberately subject-only; per-run trailers belong on [`axi run --commit-trailer`](/no-mistakes/reference/cli/#no-mistakes-axi-run), for example `--commit-trailer "Co-Authored-By: Name <email>"`.
