...
When enabled, no-mistakes suppresses each adapter's contract-defined behavior resources for every agent-driven gate step while retaining the harness's ordinary authentication and model configuration.
