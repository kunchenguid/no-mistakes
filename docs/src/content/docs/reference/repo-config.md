...
When unset, branch deltas, rebases, pull-request discovery and creation, PR summaries, and CI base-tip monitoring use the repository's discovered default branch. When set, those operations use `pr.base_branch` instead. This supports release flows where feature PRs land in an integration branch and promotion from that branch to the repository default is handled separately. New runs are always refused on the actual repository default branch and are also refused on a distinct configured base; put gated work on a separate feature branch.

The target must be a short branch name such as `quality-assurance`, not a full ref such as `refs/heads/quality-assurance`. Option-like names, whitespace, ref traversal or revision syntax, control characters, and other names Git rejects are invalid. Before a run launches an agent, no-mistakes fetches the configured branch from the upstream repository and requires it to resolve and have shared Git history with the feature branch (a usable merge base). That validated commit becomes the run's immutable base snapshot for branch-delta operations and Rebase. Normal daemon recovery reuses the retained snapshot; legacy recovery state without one freshly resolves and validates the persisted target instead of guessing a commit.

After snapshot selection, branch/PR delivery boundaries and CI boundaries for an open pull request refresh the explicit target and require its moving tip to descend from that snapshot. A deleted or unreachable target fails the current boundary closed, and a rewind, unrelated force-push, or other move away from the snapshot is refused before a branch push, PR mutation, CI repair push, or parked-CI approval can proceed. A missing, malformed, unsafe, unreachable, or unrelated target likewise fails startup with an error naming `pr.base_branch`; no-mistakes never silently falls back to the default branch for an explicit value.
...
Replacement pushes and reruns inspect persisted PR-stage evidence for the same feature branch before applying a changed trusted setting. Live provider state is authoritative: exactly one open PR against an earlier base preserves that base, including when a cached closed or merged state has since become stale; terminal PRs do not pin their old base, and legacy records without a saved base use the provider-reported target. Multiple open PRs across saved bases, an unavailable provider, malformed provider output, or any unresolved identity cause the continuity check to fail closed rather than retargeting an existing PR or creating a duplicate.
