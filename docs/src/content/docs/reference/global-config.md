...
The upload runs through the host-scoped GitHub CLI path, so no-mistakes does not extract the raw GitHub credential or construct its own authorization header. The configured credential must have repository write access and be a type the GitHub user-attachments endpoint supports.
The testing section embeds the returned image markdown or bare video URL so remote reviewers can open the media. Upload is fail-closed: any error, unsupported type, oversize file, GitHub Enterprise Server, non-GitHub forge, insufficient repository access, or unsupported credential type keeps today's rendering (a commit-pinned evidence-branch link if `store_in_repo` published, otherwise a local path) rather than a dead attachment URL. Text artifacts stay inlined as they are today.
