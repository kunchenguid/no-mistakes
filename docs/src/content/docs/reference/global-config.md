...
Session reuse fingerprints `acpx`, the raw command's resolved executable, and every command argument that names an existing file. Quoted, escaped, unresolvable, or otherwise ambiguous raw commands fail closed and start cold instead of receiving a stored session identity.
...
For ACP, a local `acpx config show` probe canonicalizes named-target configuration. Before exposing a stored ID to `sessions new --resume-session`, no-mistakes requires a local hash of the effective acpx executable, target, model, invocation environment, and either the raw target command plus its resolvable executable and file arguments or named configuration to match the stored provider binding. The binding is checked again after setup and prompt before the next session operation. Same-path executable, file-argument, environment, or configuration changes therefore start cold before prompt delivery or fail the delivered turn without replay. Raw commands whose relevant artifacts cannot be identified unambiguously start cold. Provider strings, executable paths, raw commands, and environment values do not leave the local hash.
