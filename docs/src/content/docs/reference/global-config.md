...
If a pipeline invocation fails before the selected adapter reports that its prompt may have been delivered, no-mistakes may retry it with the next available fallback. Once an adapter reports possible prompt delivery, the error is terminal at the adapter retry and fallback layers; in particular, acpx command or process failures after startup never move to another configured agent.
...
When enabled and the pipeline agent supports native session resume (Claude or Grok via `--resume`, Codex via `exec resume`, Pi via `--session <UUID>`, or Antigravity via `--conversation <id>`), each run keeps one fixer session across its review-fix turns. ACP targets through acpx run cold because acpx cannot atomically bind the complete provider implementation and configuration to the processes receiving a stored session identity and prompt.
...
ACP targets always use a fresh `exec`; they never receive a stored session identity. After prompt delivery, acpx command and process errors stop the invocation without adapter retry or agent fallback; Review and Test may still start separate fresh invocations to correct rejected structured output. Native adapters' ordinary pre-prompt resume failures may establish a new durable session. Fallbacks are recorded in the local `agent_invocations` performance record.
Session identities are minimum local resume metadata, never prompts or transcripts; Pi's own session directory retains its native transcript. Stored identities stay local and do not change the user-facing agent name. Keep Pi's session directory private, and keep any `--session-dir` or `PI_CODING_AGENT_SESSION_DIR` setting stable while a run is active so a daemon restart can find the fixer session.
