...
Only a child that was not already running before the agent last produced output counts: the agent's child processes are sampled periodically, and each output freezes a sample taken before it as the baseline, so helpers it keeps alive for the whole turn, such as the ACP agent under `acpx` or stdio MCP servers, cannot keep a hung turn alive past the stall budget, while a tool it launches right after announcing it still can.
