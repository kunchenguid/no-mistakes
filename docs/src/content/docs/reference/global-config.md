...
Session reuse fingerprints the executable named by a safely parseable raw command (a bare name or clean absolute path) as well as `acpx`. Relative, quoted, escaped, or otherwise unresolvable executable identities can be reused only by the daemon process that observed them; recovery after a daemon restart fails closed and starts cold.
...
When enabled and the pipeline agent supports native session resume (Claude or Grok via `--resume`, Codex via `exec resume`, Pi via `--session <UUID>`, Antigravity via `--conversation <id>`, or ACP targets through acpx), each run keeps one fixer session across its review-fix turns. For ACP, the first fixer turn remains the existing one-shot `exec --file -`: it captures the reported ACP identity and returns it as the run's durable fixer session without another ACP target launch.
...
The fixer session is never lent to review turns, other pipeline steps stay session-isolated in their own cold invocations, and different runs never share identities.
For ACP, a quick local `acpx config show` probe canonicalizes named-target configuration. Before exposing a stored ID to `sessions new --resume-session`, no-mistakes requires a local hash of the effective acpx executable, target, model, invocation environment, and either raw target command plus its safely resolvable executable or named configuration to match the stored provider binding. Same-path executable replacement and environment or configuration changes therefore start cold. A raw command whose executable cannot be safely identified may be reused within the observing daemon process, but its opaque process-local binding deliberately fails after daemon restart or configuration reconstruction in another process. Provider strings, executable paths, raw commands, and environment values do not leave the local hash.
A changed binding or any other stored ACP setup failure discards the stored identity and performs exactly one fresh `exec`, without a second serving-configuration probe. The first durable ACP turn is already that ordinary one-shot `exec --file -`, with one target launch. Once `sessions new` accepts a requested resume ID, the installed target may still report a replacement session during the subsequent prompt; no-mistakes persists that observed replacement. Any error after an ACP prompt process starts may follow prompt delivery and fails without retry, provider fallback, session fallback, or replay. Native adapters also never replay after possible prompt delivery; their ordinary pre-prompt resume failures may establish a new durable session. Fallbacks are recorded in the local `agent_invocations` performance record.
Session identities are minimum local resume metadata, never prompts or transcripts; Pi's own session directory retains its native transcript. acpx bridge names and provider fingerprints are opaque values isolated by trusted run scope and effective adapter identity. They and ACP identities stay local and do not change the user-facing agent name. Keep Pi's session directory private, and keep any `--session-dir` or `PI_CODING_AGENT_SESSION_DIR` setting stable while a run is active so a daemon restart can find the fixer session.
