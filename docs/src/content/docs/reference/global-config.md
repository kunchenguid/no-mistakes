...
The diagnostic identifies expiration as an **absolute wall-clock limit** and separately reports what activity was actually measured. Activity never resets or extends the hard limit. The activity snapshot is frozen at the context deadline, so process-cleanup output after cancellation cannot inflate the event count or last-activity age. Evidence resets whenever a retry or fallback starts a replacement attempt, including provider fallback, failed session resume, and OpenCode's prompt-only structured-output fallback, so the diagnostic describes only the attempt that reached the limit:
