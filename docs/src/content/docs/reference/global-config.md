...
ACP targets always use a fresh `exec`; they never receive a stored session identity. Native adapters also never replay after possible prompt delivery; their ordinary pre-prompt resume failures may establish a new durable session. Fallbacks are recorded in the local `agent_invocations` performance record.
