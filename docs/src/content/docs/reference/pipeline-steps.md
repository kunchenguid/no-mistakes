...
- `## Risk Assessment` shows the level and one bounded sentence, with longer rationale folded. `## Testing` starts with the recorded Test status plus check/evidence counts, keeps its visible summary bounded, keeps verbose text evidence behind disclosures without republishing raw recorded commands, and shows at most two trusted repository or published-evidence images inline. Arbitrary external image URLs remain links so opening a PR does not add a third-party image load. Bitbucket remains link-only and omits raw inline evidence content.
- `## Pipeline` is unchanged: it keeps the existing human-readable signature, issue/fix/verification details, and stable structured step attestation documented below. Bitbucket Cloud omits HTML-only pipeline features and the attestation comment because Cloud renders Python-Markdown and escapes raw HTML.
...
- Testing counts use the final sanitized, deduplicated evidence selection. Repository-relative and published visual paths can become linked inline images; ordinary repository files and arbitrary URLs stay `Evidence` links; content and readable UTF-8 evidence files remain collapsible on HTML-capable providers; binary, unsafe, local-only, over-budget, and provider-incompatible evidence keeps the existing safe link/local-reference/omission fallback.
