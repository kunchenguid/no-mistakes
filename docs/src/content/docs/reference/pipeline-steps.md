...
- On Azure DevOps, an empty check list after the grace period is not trusted as "no CI configured" (a build-validation policy may simply not be queued yet), so the step pauses for user approval rather than reporting a vacuous green
- On Azure DevOps, only content-influenced gates (build validation, code coverage, Component Governance security/license status checks) are watched; human review, sign-off, and attestation gates are ignored even when they report a blocking status, since no code change can satisfy them
