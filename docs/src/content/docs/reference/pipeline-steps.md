...
- When the run carries a task-tracking id ([`axi run --task-id`](/no-mistakes/reference/cli/#no-mistakes-axi-run)), that id is baked into the title after the conventional-commit format is settled, in the placement `--task-id-format` selected. It applies to the agent-drafted and fallback titles alike, on the create and update paths alike, and is skipped when the title already carries the id.
