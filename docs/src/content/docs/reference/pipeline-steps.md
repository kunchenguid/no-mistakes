...
- Bounds those agent turns with [`test_agent_timeout`](/no-mistakes/reference/global-config/#test_agent_timeout): each evidence-gathering or Test-repair invocation gets its own budget. An expired budget cancels the agent and parks for a decision with an ask-user finding rather than failing the run as a code defect. A late successful return after the deadline is not used as a Test pass. Leftover uncommitted files stay in the live worktree; a commit the timed-out agent already made is recorded locally for custody. Approving that park is a Test exception, not a silent green pass. Raise `test_agent_timeout` when targeted tests or evidence gathering routinely approach the default 30m; the shipped default is a stall bound, not slack for a long suite.
