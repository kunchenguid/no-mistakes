...
- With the default `session_reuse: true`, Claude, Codex, Grok, Pi, and Antigravity reuse one fixer session across review-fix turns. ACP targets through acpx run cold because acpx cannot atomically bind the complete provider implementation and configuration to the processes receiving a stored session identity and prompt. Unsupported agents run cold.
