...
- The drafted section follows [`pr.style`](/no-mistakes/reference/global-config/#prstyle). Under the default `rich` style the agent writes a `## Summary` prose section followed by emoji-led `###` subsections - production evidence, before and after, regression evidence, deliberate scope decisions, known gaps - with every source of evidence linked inline and any link it cannot substantiate omitted rather than guessed. Under `minimal` it writes the terse `## What Changed` bullet list instead. The drafting fallback always uses `## What Changed`.
