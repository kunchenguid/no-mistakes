...
[`commands.test`](/no-mistakes/reference/repo-config/#commandstest) owns the configuration contract for any explicit configured test command.
...
**Auto-fix:** the agent receives the previous test findings plus any per-finding user notes, any selected user-authored findings from the TUI or AXI interface, and the shared [finding decision history](#finding-decision-history), including earlier fix summaries for this step. Repair mode reproduces the specific failure, applies a root-cause fix, and re-runs only focused verification - not a complete-suite confirmation - then the step's configured test command (if any) and evidence path run again.
