...
- When the generated PR body approaches GitHub's 65536-character body limit, older `## Pipeline` update rounds are omitted at clean update boundaries so the essential intent, change summary, risk, and testing sections remain intact.
- The omission marker in the PR body points reviewers to the run log for the full pipeline history.
