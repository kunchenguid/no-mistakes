...
- When the reviewer's final output is rejected, reruns a fresh, session-free review with the same prompt plus a note quoting the validation error, up to three attempts in total. These are schema/output retries inside one review execution; they do not create pipeline review rounds or spend the fixer budget.
...
- With the default `session_reuse: true`, Claude, Codex, Grok, Pi, and Antigravity may resume the durable fixer session for Review's one fix turn after daemon recovery; a resume failure retries that same turn in a fresh fixer session, and unsupported agents run cold
- Bounds each agent turn independently with [`review_agent_timeout`](/no-mistakes/reference/global-config/#review_agent_timeout): the optional fixer and its fresh, session-free rereviewer each receive the full wall-clock limit, as does each schema/output retry inside either review execution; reaching the hard limit cancels that agent and fails the step with measured last-activity or no-output evidence rather than leaving the run active indefinitely
...
**Approval:** required if any finding has severity `error` or `warning`. Findings with `action: ask-user` always pause for an explicit decision and are never silently fixed by `axi run --yes`. This is for findings that challenge the author's intent, or whose smallest honest remedy would extend the change rather than correct it, not routine correctness, reliability, or security fixes. Findings with `action: no-op` are informational only.

Review has one bounded correction cycle per run: the initial review, at most one bundled fixer execution, one full rereview, then an explicit approve or skip decision if blocking findings remain. The durable step-round history enforces that cap across daemon restarts. AXI renders the current review round, fixer runs used, limit, remaining, and the actions still allowed.

`auto_fix.review` controls only who spends that one fixer execution. With the default `0`, blocking findings park first and an operator may select one Fix response. Any positive value automatically spends the execution on eligible `auto-fix` findings; values above `1` do not permit more Review fixer rounds. After the rereview, another Fix response is rejected without invoking an agent or changing the durable review cycle.

**Auto-fix:** the agent receives all selected findings in one bundled payload, plus any per-finding user notes, selected user-authored findings from the TUI or AXI interface, and the shared [finding decision history](#finding-decision-history), including earlier fix summaries for this step.
The fixer addresses every selected instance before running one focused verification limited to the changed area. It prefers the deeper architectural cause and simplification over machinery that suppresses symptoms.
...
The dedicated Test and Lint steps after review remain the authoritative gates.
Follow-up review uses the history to avoid re-reporting user-ignored findings unless the code now has a materially different problem.

**Default automatic spend:** `0`. **Total fixer executions:** `1`.
