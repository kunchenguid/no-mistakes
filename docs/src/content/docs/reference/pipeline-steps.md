...
When a finding can be resolved by removing a code path the intent does not strictly require (an extra acceptance or matching branch, a fallback, an alias, a second definition of something the code already defines once, or handling for an input nobody intends), it removes that path instead of validating, hardening, or documenting it, judging what the intent requires against the run's user intent when present and otherwise against the change's own stated purpose. The guard against reverting the author's intentional code protects only code the intent requires; genuine doubt about whether the intent requires a path leaves it in place and reports the finding unresolved.
