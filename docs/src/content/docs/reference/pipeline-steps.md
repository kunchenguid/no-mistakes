...
- The step records the exact tests and checks it exercised in a `tested` array, may include a short natural-language `testing_summary`, and includes an `artifacts` array for reviewer-visible evidence; `path` artifacts are normalized before publication, `url` artifacts must be externally visible, and `content` artifacts should be short logs or command output shown directly in the PR.
- Evidence is collected in a temporary run directory so text artifacts retain inline PR rendering. Image publication is opt-in; its storage, validation, and fallback behavior is defined in the [global configuration reference](/no-mistakes/reference/global-config/#testevidence).
...
- Evidence artifacts render compactly in PR bodies: repository-relative `path` artifacts and `url` artifacts become `Evidence` links, `content` artifacts appear in collapsible details blocks, published GitHub images render inline from path-segment-escaped immutable `blob/<sha>/...?raw=1` URLs at the pushed commit, readable UTF-8 text files from the temporary evidence directory are embedded inline with truncation for large files, and missing, unsupported, oversized, or unpublished artifacts render as path-free explanations
- When GitHub fork routing is configured, those immutable evidence URLs target the fork that received the push rather than the parent repository
