...
- Only after the remote and gate mirror settle, atomically records the exact delivered commit as both the run head and successful-push binding; a failure before that record leaves publication unsettled for a later retry
...
- Settles the local gate mirror before atomically recording the published head and push binding. If publication stalls after the remote moved, the live monitor records the exact attempted commit in memory and retries only that commit before evaluating check state or consulting or charging the auto-fix budget; a merely different clean worktree head is not evidence of a pending publication. The retry verifies the remote's up-to-date head and completes publication once the mirror works. After a daemon restart the in-memory marker is gone, so the next fix attempt settles the repair at the cost of that attempt rather than claiming publication evidence the restarted process does not have
