...
- On GitHub, also treats a job that failed before any repository step ran - its setup/action-resolution phase failed, such as a "Failed to resolve action download info" / HTTP 503 action-download outage - as a provider-side outcome rather than a code verdict, read structurally from the job's own setup-step conclusion (never from log text) so it can never mask a real test or lint failure
