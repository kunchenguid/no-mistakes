...
An analyzer response is usable only when it satisfies its step's output contract. Missing, malformed, or semantically incomplete structured output fails that step rather than producing a clean result that yolo mode can auto-resolve.
