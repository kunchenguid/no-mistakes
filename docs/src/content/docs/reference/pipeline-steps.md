...
- For GitHub, when `providers.github.draft_pull_requests` is enabled, new PRs are created as drafts (`gh pr create --draft`); for GitLab, when `providers.gitlab.draft_pull_requests` is enabled, new MRs are created as drafts (`glab mr create --draft`); for Bitbucket, when `providers.bitbucket.draft_pull_requests` is enabled, new PRs are created as drafts (`"draft": true` in the create-PR API request); for Azure DevOps, when `providers.azuredevops.draft_pull_requests` is enabled, new PRs are created as drafts (`az repos pr create --draft true`); existing PRs/MRs are not toggled between draft and ready
