...
- Settles the local gate mirror before atomically recording the published head and push binding. If mirror settlement fails after the remote moved, the monitor retries that already-created repair before consulting or charging the auto-fix budget; the retry verifies the remote's up-to-date head and completes publication once the mirror works
