...
- On GitHub, combines the PR check rollup with Actions workflow runs for the exact current PR head commit, so a workflow rejected during validation before it creates a job or check-run still blocks readiness
