...
- The PR stage exclusively owns the complete branch-scope description. Its drafting agent returns a conventional title, a two-sentence intent summary, grouped acceptance criteria, and one to three `## What Changed` bullets from the actual final diff after local mutating stages finish. Go code—not the agent—owns the final section order and disclosure markup. Legacy or partially malformed drafting responses keep valid title/body fields and use a deterministic overview fallback.
- When user intent is available, the visible `## Intent` stays short and one collapsed Acceptance criteria block lists brief human-readable checks; each check has nested optional detail, and a final collapsed item escrows the complete sanitized source intent. Under a hard provider cap the complete-context item is removed atomically and replaced by an explicit omission marker rather than being cut into malformed markup. Bitbucket Cloud receives bounded plain Markdown criteria and links instead of raw HTML disclosures.
- `## What Changed` is normalized to one to three top-level bullets. Same-section supporting prose or fallback changed-path/status evidence stays behind bounded progressive disclosure instead of expanding the visible description.
- `## Risk Assessment` shows the level and one bounded sentence; on HTML-capable providers the longer rationale is folded behind a disclosure, while Bitbucket Cloud shows the remaining rationale as an indented continuation instead. `## Testing` starts with the recorded Test status plus check/evidence counts, keeps its visible summary bounded, keeps verbose text evidence behind disclosures without republishing raw recorded commands, and shows at most two trusted repository or published-evidence images inline. Arbitrary external image URLs remain links so opening a PR does not add a third-party image load. Bitbucket Cloud stays link-only, omits raw inline evidence content, and reports a count for evidence items beyond its bounded list.
- `## Pipeline` is unchanged: it keeps the existing human-readable signature, the auto-fix issue -> fix -> verification narrative built from captured fix summaries, re-check success text, and still-open findings, the recorded commands listed under its Test details, and the stable structured step attestation documented below. Bitbucket Cloud omits HTML-only pipeline features and the attestation comment because Cloud renders Python-Markdown and escapes raw HTML.
...
- Intent, acceptance summaries, `## What Changed`, risk, and testing remain ahead of pipeline history. New disclosure units are removed only at balanced boundaries; existing pipeline-history truncation continues to keep the newest rounds when possible and adds an explicit marker.
- Testing counts use the final sanitized, deduplicated evidence selection. An image artifact is inlined only when the renderer itself derives absolute `http(s)` source and raw targets for its recorded `path` - GitHub blob and raw URLs, or published-evidence URLs; where those bases do not exist (GitLab, Azure DevOps, an unrecognized provider) the artifact stays a link rather than an image reference that would not resolve. Ordinary repository files and arbitrary URLs stay `Evidence` links; content and readable UTF-8 evidence files remain collapsible on HTML-capable providers; binary, unsafe, local-only, over-budget, and provider-incompatible evidence keeps the existing safe link/local-reference/omission fallback.
...
- For Azure DevOps, the PR description is capped at 4000 characters (UTF-16 code units, matching .NET's measurement): the agent is told about the cap and asked to keep `## What Changed` compact; when the assembled body overruns, it preserves Intent, acceptance summaries, What Changed, Risk Assessment, Testing, and the newest Pipeline update before removing older Pipeline history. Testing is omitted only when those higher-priority sections still cannot fit with the newest update; a final connector-level clamp truncates with a visible marker as a last-resort backstop.
