...
- Waiting for a check to register only ends somewhere. On GitHub, when the check list is empty and the repository does not declare `no_ci: true`, the step asks the provider what CI the repository defines: the workflows GitHub has registered, plus the workflow files in the commit under test, which covers the pull request that adds the repository's first workflow. If both are empty, no check can ever register, so the step stops and pauses for user approval with a finding that says so and points at `no_ci: true` as the durable answer. This is explicitly **not** a pass - nothing tested the commit - and it never establishes readiness. Any other answer, including a provider that cannot be read and a provider with no such lookup, keeps the existing waiting behavior unchanged. The lookup covers GitHub Actions only, so a repository whose checks come from an external service that has not yet posted its first status is reported as having no CI; that outcome is a question to answer, never a green verdict
