...
Review and Test prompts share bounded tool-call guidance: when the agent supports Code Mode or an exec cell and has two or more independent read-only shell or tool calls, it is asked to run them concurrently in one exec cell with `Promise.allSettled` or `Promise.all`, inspect every result, and keep dependent, ordering-sensitive, and mutating work sequential. The guidance also asks the agent not to split one such batch across separate outer tool calls; it is prompt steering, not runtime enforcement.
