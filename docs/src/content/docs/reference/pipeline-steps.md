...
- If existing-PR discovery fails or its provider response cannot be decoded and validated as a PR listing for the configured repository, stops instead of treating the result as no PR and creating a duplicate. GitHub accepts an otherwise-valid PR URL at a renamed repository path only after the [authenticated repository-identity proof](/no-mistakes/reference/cli/#repository-rename-continuation); PR discovery does not migrate a persisted push binding.
