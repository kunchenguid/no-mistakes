...
- For GitHub fork routing, requires the authoritative run intent to contain exactly one destination as a dedicated `PR destination: owner/repo` line. Before any PR lookup, update, or creation, the step verifies that destination against the parent repository selected from `origin`; a missing, conflicting, malformed, or fork destination fails the step instead of publishing. An explicit parent destination preserves fork-head-to-parent contributions.
