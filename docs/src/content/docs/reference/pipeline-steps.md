...
- On GitHub, GitLab, and Bitbucket Cloud, an empty check list after the grace period means "no CI configured" and reports the PR ready. On Azure DevOps an empty check list is ambiguous (build-validation policies routinely report no evaluation for an unvalidated PR), so it fails safe: the run pauses for user approval with an `ask-user` finding rather than reporting a vacuous ready
