...
- Waits a 60s grace period before trusting empty results (CI checks may not have registered yet); after the grace period, a PR with no configured checks is treated as healthy and surfaces the same ready signal as passing checks while monitoring continues
