...
- If attestation interrupts a clean, continuity-proven CI repair, records its exact retained head and review authority against the original run and a run-specific gate ref. An explicit `fix` response retries that bound publication before invoking a fixer, preserving the attestation and force-push guards. Authentication failure during this retry keeps it parked rather than skipping attestation. Dirty, moved, unbound, or conflicting state is refused; a newly trusted policy requiring Review also parks rather than silently starting another review. Recovery may restore this bound head mismatch without changing the recorded run head. Older runs without that binding are not automatically adopted from prose findings or ancestry alone.
