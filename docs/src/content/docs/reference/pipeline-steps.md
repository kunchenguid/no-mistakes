...
- After the test suite passes, a language-pluggable coverage sub-step mechanically verifies that changed source files have non-zero coverage on their added (diff) lines. It re-runs the suite instrumented through per-language providers (Go via `go test -cover`, JS/TS via `npx c8`, Swift over SSH on a Mac build executor), so it only fires when tests already pass and coverable changed files exist, and it emits one `warning`/`ask-user` finding (`uncovered-changed-lines:<lang>:<path>`) per changed source file that adds code no test exercises. A brand-new untested file still fires because all of its lines are added and uncovered. It is gated by `NO_MISTAKES_COVERAGE_CHECK` (default on when a `go.mod`, `package.json`, or Swift manifest is present; see [Environment Variables](/no-mistakes/reference/environment/)) and degrades to a logged no-op on provider errors — it never blocks the pipeline on a collection failure.

**Approval:** test findings with `action: ask-user` pause for approval, including missing-evidence warnings for user intent and coverage findings for changed source files with no test coverage. `action: auto-fix` findings stay eligible for the fix loop. `action: no-op` findings are informational only.
