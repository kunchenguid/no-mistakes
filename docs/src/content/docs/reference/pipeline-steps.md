...
- With the default `session_reuse: true`, Claude, Codex, Grok, Pi, and Antigravity reuse one durable fixer session across review-fix turns with the effective `agent`; [`agent_config.<agent>.review_fix`](/no-mistakes/reference/global-config/#agent_config) can give that session a role-specific profile on the same harness; a resume failure retries the same fix turn in a fresh fixer session, and unsupported agents run cold
