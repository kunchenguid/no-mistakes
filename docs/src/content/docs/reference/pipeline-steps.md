...
- After immediate publication is remotely verified and durably bound, treats a late local gate-mirror synchronization failure as a warning and keeps monitoring the published repair instead of spending another fix attempt. The ordinary Push step still fails on the same mirror error so a retry can reconcile the mirror
- When revalidation is enabled, restarts at Review only: Intent and Rebase keep their results, steps already skipped for the run stay skipped, the run id is unchanged, and the durable auto-fix attempt count carries across. Earlier cycles remain in the run's round history; the step's own status shows the latest cycle
