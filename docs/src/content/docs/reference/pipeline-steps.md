...
- The evidence prompt warns agents that a live TUI scenario driven through a pseudo-terminal should give the pty a non-zero window size (`TIOCSWINSZ`) before the TUI's first grid read and drain the master. `script(1)` and a bare `forkpty()`/`pty.fork()` from a non-tty parent otherwise yield a 0x0 grid; the TUI then exits immediately with a symptom such as `terminal reported a zero-sized grid` and never registers, so a live UI check can silently become a fake while the pipeline still passes. This is agent guidance, not a machine-enforced evidence requirement.
