...
- Immediately before GitHub creates or updates a PR, the connector applies one final local-identity privacy check to the complete title and body. Home roots become `~`; concrete generated temporary paths, Treehouse managed-copy roots, and Pi or Herdr session identifiers become stable Markdown-safe placeholders. Generic examples such as `/tmp/example`, ordinary URLs, commit SHAs, Markdown structure, and the pipeline attestation remain byte-stable. A managed-copy path that cannot be transformed confidently is refused without sending or logging its raw value. Both initial creation and every full-body update (including body-only attestation restamps) use this boundary before `gh` receives argv or stdin. The earlier assembled-content home scrub remains provider-neutral defense in depth, and all replacements are non-growing because provider length caps have already been applied.
