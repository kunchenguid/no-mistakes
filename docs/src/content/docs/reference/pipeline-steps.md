...
- For GitHub fork routing, after provider availability succeeds and before any PR lookup, update, or creation, requires the authoritative run intent to start with a dedicated `PR destination: owner/repo` line and verifies it against the selected parent PR repository. A missing, conflicting, malformed, or fork destination fails the step instead of publishing; an explicit parent destination preserves fork-head-to-parent contributions.
