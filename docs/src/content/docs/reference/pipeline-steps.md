...
Because a fix round only rereviews the fix diff, unresolved review findings are carried across the step's rounds until they are explicitly resolved; [unresolved findings across rounds](/no-mistakes/concepts/auto-fix/#unresolved-findings-across-rounds) owns that behavior.
