...
- A per-run `--base-branch` that disagrees with an existing PR's live forge base retargets that PR (GitHub `gh pr edit --base`, GitLab `glab mr update --target-branch`, Gitea `tea api` PATCH) before updating title and body. A provider that cannot retarget fails closed instead of rewriting the PR against a base it did not move. A repo-config `pr.base_branch` change still does not retarget.
