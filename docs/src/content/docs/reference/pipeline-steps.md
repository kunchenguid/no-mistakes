...
These correction commits, and the Push step's commit of leftover agent changes, are machine-authored records of what the pipeline's own fix agents and formatter produced, so they are created with the repository's commit hooks suppressed: `--no-verify` plus an empty `core.hooksPath` for that one invocation, which also covers hooks Git runs regardless of `--no-verify`, such as `prepare-commit-msg`. Without that, a run dies on setup state rather than on the change under review, because the disposable run worktree has none of the generated untracked runtime files a tracked hook may need - a repository whose shared config sets `core.hooksPath=.husky` while a tracked hook sources the generated `.husky/_/husky.sh` is the canonical case. The override lives only in that single `git commit` invocation; nothing is written to the repository, your Git configuration, or the daemon environment, and CI repair commits plus every commit you author yourself still run your hooks normally. The Review, Test, Document, Lint, Push, PR, and CI gates remain the authoritative checks on what these commits contain.
