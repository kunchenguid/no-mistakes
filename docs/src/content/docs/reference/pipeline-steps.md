...
- PR body includes a `## Intent` section when user intent is available, the final-diff `## What Changed`, and a compact `## Pipeline` section. Pipeline entries expose step status and outcome counts only; Review is rendered as completed without its earlier risk rationale. The `## Pipeline` section can be omitted entirely with [`pr.include_pipeline_summary: false`](/no-mistakes/reference/global-config/#princlude_pipeline_summary).
