...
- On GitHub, treats an `action_required` workflow as not run and waiting for a maintainer: it is never a CI-fix target or provider-rerun candidate, and it parks at an approval gate with that diagnosis when no genuine failure also needs handling
