...
- Evidence is collected in a temporary run directory so text artifacts retain inline PR rendering. By default, supported images are fully decoded, bounded, copied under `<test.evidence.dir>/<branch-slug>` with content-addressed names, staged from the recorded image manifest, and rendered inline from the exact pushed commit. Unsafe, symlinked, disabled, missing, unsupported, oversized, or failed publication falls back without exposing a local path.
