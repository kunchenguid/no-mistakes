...
At admission, a run captures the registered checkout's explicit worktree-local or repository-local `commit.gpgsign` setting as a canonical tri-state (`true`, `false`, or unset) and persists it for the run. A rerun inherits the selected run's persisted setting when present instead of recapturing the checkout, and daemon recovery reapplies any persisted setting to the run worktree. Explicit `true` and `false` values are also propagated command-locally through pipeline correction and CI repair commits, rebases, and agent subprocesses; unset leaves ordinary Git configuration precedence in effect. Setting `git config --local commit.gpgsign false` in the registered checkout therefore keeps autonomous gate commits unsigned without changing global Git configuration.
