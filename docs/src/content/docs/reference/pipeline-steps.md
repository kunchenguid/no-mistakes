...
The run worktree also inherits an explicitly repository-local `commit.gpgsign` value from the registered checkout. Setting `git config --local commit.gpgsign false` therefore makes autonomous gate commits honestly unsigned without changing global Git configuration, while an explicit `true` or an unspecified local value retains normal signed/default Git behavior.
