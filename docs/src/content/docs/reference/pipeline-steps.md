...
**Approval:** required if any finding has severity `error` or `warning` (`error` only under [`review.gate_severity: error`](/no-mistakes/reference/repo-config/#reviewgate_severity), where warnings are reported but never park or spend a fix round). Findings with `action: ask-user` pause for approval instead of entering the normal auto-fix loop, unless [`review.auto_fix_ask_user: true`](/no-mistakes/reference/repo-config/#reviewauto_fix_ask_user) lets the automatic loop fix them while fix rounds remain. [`review.max_fix_rounds`](/no-mistakes/reference/repo-config/#reviewmax_fix_rounds) caps automatic and gate-driven fix rounds together: at the cap the step parks with every outstanding finding and refuses a further `fix` (`fix_rounds_exhausted`). This is for findings that challenge the author's intent, or whose smallest honest remedy would extend the change rather than correct it, not routine correctness, reliability, or security fixes that may need to re-add a small amount of deleted logic. With the default `auto_fix.review: 0`, blocking review findings park for approval even when their action is `auto-fix`; setting repo or global `auto_fix.review` above `0` re-enables the automatic review fix loop for eligible `auto-fix` findings. Findings with `action: no-op` are informational only. The shared [finding-action model](/no-mistakes/concepts/auto-fix/#finding-actions) owns the behavior for a missing `action`.
