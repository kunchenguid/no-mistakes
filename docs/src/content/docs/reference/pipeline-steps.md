...
- Evidence is always collected under the temporary `no-mistakes-evidence/<runID>` directory, outside the worktree, so artifacts never enter the branch being validated. With [`test.evidence.store_in_repo: true`](/no-mistakes/reference/global-config/#testevidence), the PR step publishes that directory to the repository's orphan evidence branch under `<test.evidence.dir>/<branch-slug>` and links the artifacts from the PR body; when publication is refused the artifacts stay local-path references.
...
- Evidence artifacts render compactly in PR bodies: repository-relative `path` artifacts and `url` artifacts become `Evidence` links, `content` artifacts appear in collapsible details blocks, GitHub PRs convert repository-relative paths to blob URLs and published evidence to blob URLs pinned to the evidence commit, readable UTF-8 text files from the temporary evidence directory are embedded inline with truncation for large files, and binary, visual, or over-budget local artifacts render as non-link local file references
