...
- Applies the configured draft setting only when it creates a GitHub, GitLab, Bitbucket Cloud, or Azure DevOps PR/MR; it never changes the draft state of an existing PR/MR. See the [global](/no-mistakes/reference/global-config/#providersgithubdraft_pull_requests) and [per-repo](/no-mistakes/reference/repo-config/#providersgithubdraft_pull_requests) config references.
