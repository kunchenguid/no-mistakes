...
- Only after the remote and gate mirror settle, atomically records the exact delivered commit as both the run head and successful-push binding; until that database write succeeds, the durable database head and binding remain unchanged. For CI repairs, a failure after remote verification creates the bounded retry described under [CI](#ci)
...
- Settles the local gate mirror before atomically recording the published head and push binding. If publication stalls after the remote is verified at the repaired head, the monitor durably records that exact commit and retries only it before evaluating check state, timeout, or the auto-fix budget; a merely different clean worktree head or a failure before remote verification is not evidence of a pending publication. The retry verifies the remote's up-to-date head and completes publication once local settlement works. Its durable attempt count survives daemon restart. After three unsuccessful retries, the run parks with a finding that names the remotely verified head and warns that rerunning is unsafe until the gate mirror and run record are reconciled
