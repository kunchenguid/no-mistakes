...
- Invokes the user-installed `code-review` skill through the configured agent's normal skill discovery. The skill remains user-owned and is not bundled with no-mistakes; its current installed version is the canonical contract for scope resolution, source discovery, review lanes, evidence verification, severity, and completeness
- Supplies the base and head as exact comparison endpoints, plus pipeline context, trusted path instructions, intent, and prior-round history, so the non-interactive review does not need to infer a merge base or ask which change to inspect
- Adapts the skill's completed result to no-mistakes' structured findings: Critical and High verified findings become `error`, Medium and Low become `warning`, verified nits become `info`, and material unresolved risks or incomplete active lanes park for user review. No-mistakes still owns finding actions, gate transitions, durable state, retries, and response handling
...
- If the configured runtime cannot discover or load `code-review`, requires an `error` / `ask-user` finding instead of substituting a built-in review policy, so the missing canonical review cannot become a clean pass
...
- The skill may run its own inspect-only candidate-review passes and evidence checks. Those are internal to the review contract and do not replace no-mistakes' pipeline orchestration or later Test and Lint gates
- Agent returns translated findings with severity (`error`, `warning`, `info`), file location, description, and an `action` (`no-op`, `auto-fix`, `ask-user`)
