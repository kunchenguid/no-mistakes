...
- PR body includes a `## Intent` section when user intent is available, an agent-authored `## What Changed`, and deterministic, regenerated `## Risk Assessment` and `## Testing` sections; there is no `## Pipeline` section and no self-referential no-mistakes signature (see `AGENTS.md`, "Generated PR Body Sections")
- Generated PR bodies are capped at 63,488 bytes, leaving a 2 KB safety buffer below GitHub's 65,536-character body limit.
- When the full body would exceed that cap, the PR step first drops the `## Testing` section (it embeds artifact and log content and is effectively unbounded) so Intent, What Changed, and Risk Assessment are preserved; if it still overruns, it truncates at line or section boundaries and adds an explicit marker.
