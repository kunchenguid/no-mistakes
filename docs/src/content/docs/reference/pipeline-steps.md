...
- Bounds those agent turns with [`test_agent_timeout`](/no-mistakes/reference/global-config/#test_agent_timeout): each evidence-gathering or Test-repair invocation gets its own budget. An expired budget cancels the agent and parks for a decision with an ask-user finding rather than failing the run as a code defect. The configured baseline's result from the same execution stays on that park; after a cut repair turn the baseline runs once more first, so the park reports the state the cut left. A late successful return after the deadline is not used as a Test pass. A commit the timed-out agent already made is recorded locally for custody, unless an unfinished rebase or merge leaves only a partial HEAD. While the worktree holds commits or uncommitted changes no Test turn validated, the park names them with the commands to inspect them and refuses approval, because Document and Push would commit and publish them; a fix response validates them. Otherwise approving that park is a Test exception, not a silent green pass. A fix response whose selection holds only the budget-cut findings re-runs validation without a repair turn. Raise `test_agent_timeout` when targeted tests or evidence gathering routinely approach the default 30m; the shipped default is a stall bound, not slack for a long suite.
