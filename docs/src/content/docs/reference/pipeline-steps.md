...
- A proposed CI repair is refused, uncommitted, when its effect is to undo work the branch deliberately did: a file it returns to byte-identical pre-branch content, or a line that exists exactly once in the pre-branch file, that the branch removed and the repair restores. Both are measured against the fork point with git alone, before anything is staged, so the branch head and the run state are untouched and the proposed changes stay in the run worktree. The step parks with the evidence. Answering the gate with a fix selection authorises exactly the listed reversion: that response runs another repair round rather than committing the one you read, so anything the replacement round would undo beyond the list parks again with its own evidence. This exists because a repair that undoes the branch's own work is invisible in the branch's `base..head` diff, so nothing downstream - including the re-review that follows every CI repair - can see it
...
- Carries the run's recorded decisions into the CI repair prompt, so a repair round can see the findings a person already ruled on rather than re-deciding them
