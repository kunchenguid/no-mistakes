...
- If existing-PR discovery fails or its provider response cannot be decoded and validated as a PR listing for the configured repository, stops instead of treating the result as no PR and creating a duplicate. GitHub is the narrow rename exception: when an otherwise-valid same-host PR URL carries a new owner/repository path, discovery accepts it only if authenticated API reads of both the configured path and returned canonical path report the same positive immutable repository ID and both identify that returned path as canonical. Redirects, names alone, missing or malformed identity, API failure, a different host, or a different repository/fork still refuse.
