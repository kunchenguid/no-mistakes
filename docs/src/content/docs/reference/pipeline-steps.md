...
- Generated public prose also drops the known operator-address forms `Captain,` and `Captain:` (case-insensitively) at sentence, list, and label boundaries. This covers PR titles, summaries, risk assessments, findings, fix summaries, and evidence labels, including fallback bodies. The same rule applies to wizard commit suggestions and the generated summary in [auto-fix commit subjects](/no-mistakes/reference/global-config/#commitfix_message). Domain phrases such as `the captain, crew and ship`, quoted dialogue, code, captured output, and pipeline markers retain their text; literal commit-template text and repository files are not rewritten. This is a bounded address rule, not a detector of arbitrary names or attribution claims.
