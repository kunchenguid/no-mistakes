...
- If reviewer-visible evidence specifically requires the same-run deployed PR preview, which cannot exist before Push and PR, Test records a `deferred_evidence` request instead of asking the user to waive the missing evidence. A missing local application runtime is not itself an approval blocker when the deployed preview can provide the requested rendered surface later.
...
- After CI reaches its ready condition (all reported checks pass, or no checks register after the grace period), consumes any preview-only `deferred_evidence` recorded by Test: the evidence agent opens the deployed preview for the exact PR and commit, captures the rendered surface without using a local `APP_BASE_URL` or starting a local server, uploads a reviewer-visible artifact to a public HTTP(S) URL, merges it into the Test result, and regenerates the PR's `## Testing` section. When repository instructions require machine-only login or upload credentials, the agent may read only those named values from env files under the registered working clone; that clone is never used as source and must not be modified. The run cannot become `checks-passed` until this succeeds; an unavailable preview or missing public artifact pauses for approval without consuming the deferred request.
