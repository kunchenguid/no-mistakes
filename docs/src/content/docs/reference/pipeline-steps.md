...
- The Test evidence prompt advises that a TUI driven through a pseudo-terminal should get a non-zero window size (`TIOCSWINSZ`) before it reads its grid, with the master drained. That guidance lives in the agent prompt, not in this page: a reference-doc line never reaches the evidence agent. `script(1)` and a bare `forkpty()`/`pty.fork()` from a non-tty parent typically yield a 0x0 grid; the TUI then exits immediately with a symptom such as `terminal reported a zero-sized grid` and never registers, so a live UI check can silently fake-pass. This is prompt advice, not a findings-contract check.
