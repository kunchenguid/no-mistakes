...
- With the default `session_reuse: true`, Claude, Codex, Grok, Pi, and Antigravity reuse one durable fixer session across review-fix turns, using global [`review_fix_agent`](/no-mistakes/reference/global-config/#review_fix_agent) when configured and otherwise the effective `agent`; a resume failure retries the same fix turn in a fresh fixer session, and unsupported agents run cold
