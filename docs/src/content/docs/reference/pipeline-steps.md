...
- By default, evidence is stored under the temporary `no-mistakes-evidence/<runID>` directory. For GitHub PRs, local image and video evidence is uploaded to secret gists when `test.evidence.upload_to_gist` is true, with local path rendering as a non-failing fallback. With `test.evidence.store_in_repo: true`, evidence is stored under `<test.evidence.dir>/<branch-slug>` inside the worktree, staged during push, and published with the branch. Unsafe, symlinked, or Git-ignored evidence directories fall back to temporary storage for that run.
