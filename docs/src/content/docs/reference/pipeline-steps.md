...
- On GitHub, collapses repeated same-name check runs to the newest run that provider timestamps or Actions run identity can order. Same-named commit status contexts remain independent requirements, and check runs whose order cannot be established remain visible so readiness fails closed
