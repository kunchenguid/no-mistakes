...
- For changed integrations with external executables, services, SDKs, configuration sources, or environment-dependent runtimes, verifies actual dependency semantics, traces flag/environment/config/default/fallback precedence, and checks the environment of the process that consumes the dependency rather than assuming the review shell is representative
...
Runs **targeted** local validation through a configured command, or gathers agent-led evidence when no command is configured.
...
- If `commands.test` is configured, it runs exactly once in a normal Test step via the platform shell (`sh -c` on POSIX, `cmd.exe /c` on Windows), with output streamed into the live step log and retained for failure findings. A successful command completes Test without launching an evidence agent or repeating tests. Configure a **targeted** command here (see repo-config); do not treat this field as CI-parity complete-suite configuration.
