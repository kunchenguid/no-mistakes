...
- By default, evidence is stored under `<test.evidence.dir>/<branch-slug>` inside the worktree, staged during push, and published with the branch. Supported images are validated, bounded, content-addressed, and rendered inline from the exact pushed commit. Unsafe, symlinked, Git-ignored, disabled, missing, unsupported, oversized, or failed publication falls back without exposing a local path.
