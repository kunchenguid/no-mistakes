...
When the run was started with one or more [`axi run --commit-trailer`](/no-mistakes/reference/cli/#no-mistakes-axi-run) values, no-mistakes adds the persisted trailers to every pipeline-generated commit in the original commit operation: shared Review/Test/Document/Lint fix commits, CI repair commits, and Push-step leftover-change commits.
