...
- With the default `session_reuse: true`, Claude, Codex, Grok, Pi, Antigravity, and ACP targets through acpx reuse one fixer session across review-fix turns. An ACP fixer's first turn uses the unchanged one-shot `exec --file -` and captures its reported identity with one target launch. Before exposing a stored ID to `sessions new --resume-session`, later turns require the local hashed binding of effective executables, raw-command file artifacts, environment, target, model, raw override, and named configuration to match, then recheck that binding between session operations. Changed or unverifiable bindings and stored ACP setup failures discard the identity and perform exactly one cold `exec`; prompt-time errors never replay. If setup accepts the requested ID but the target reports a replacement during prompt, that observed identity is persisted. Unsupported agents run cold, and ACP bridge names and provider bindings remain opaque local isolation data.
