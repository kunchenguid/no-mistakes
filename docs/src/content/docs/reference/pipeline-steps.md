...
Pipeline steps do not treat missing, malformed, or semantically incomplete structured analyzer output as a clean result.
Such output never creates a gate that unattended AXI mode can accept.
Review and Test support bounded, correction-only turns as described below; unrecoverable output or an exhausted correction bound stops the affected step.
...
**Output correction:** an otherwise identifiable report with incomplete finding fields can receive up to two fresh correction-only turns, separate from `auto_fix.review`.
Common `title`/`body`/`code_location` reports are preserved as descriptions and locations, but priority and confidence numbers are never mechanically mapped to severity.
The correcting agent must supply each missing severity from the original observations; unsupported classifications still fail validation.
Missing actions and scopes conservatively become `ask-user` and `source`, and `tested: false` means no tests were run.
Corrections cannot remove, merge, reorder, rewrite, or reclassify existing findings, lower the reported risk, or invent testing observations.
Duplicate keys, competing findings reports, conflicting field shapes, missing observations, and invalid risk assessments fail closed with diagnostics pointing to the review log.
The correction prompt supplies only the rejected data, validation errors, and a preservation template, and instructs the agent not to use tools or rerun the review; this is prompt steering, not an OS sandbox.
...
- Each scenario records `name`, `result` (`pass`, `fail`, or `untested`), `live`, and `evidence`; the overall `verdict` is `go`, `no-go`, `inconclusive`, or `no-surface`.
  A non-empty `reason` is required for `untested`; it is optional for a live pass or fail, whose evidence explains the result.
  `live` is true only when that scenario was driven against the real running product in this run.
  Unit tests, stubs, mocks, recorded fixtures, and code inspection are not live.
  A scenario the machine cannot drive is `untested` with a reason identifying either the unavailable capability or the absence of a live product surface, never a guessed pass.
  `no-surface` is for a change with no runtime product no-mistakes can drive live (CI-workflow-only, docs-only, a pure non-runtime refactor, or anything else with no live-exercisable scenario); every scenario must be untested and not live, or the payload is rejected rather than treated as a skipped live validation.
- The evidence payload must contain a non-empty scenario list and a verdict, use the exact lowercase vocabulary above, include the required scenario fields, provide evidence for each pass or fail, and avoid a fail scenario with a verdict other than `no-go`.
  Findings from runs recorded before this contract remain readable and render without scenario details.
- An evidence payload that violates that contract is rejected.
  A fresh, correction-only analyzer invocation receives the rejected payload and specific validation errors as untrusted data, including the complete identifiable JSON object when a native adapter rejected its schema.
  It is instructed not to use tools, rerun scenarios, or perform external operations, and to preserve supported observations and downgrade unsupported pass or fail claims to `untested` rather than inventing evidence.
  The step allows two extra correction attempts after the first invalid payload.
  Exhausting that bound fails the step; the contract itself is not relaxed.
  A valid payload, including a mix of live passes without reasons and untested scenarios that carry a reason, is accepted on the first attempt.
