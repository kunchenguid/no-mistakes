...
Historical unpublished recovery requires complete, target-bound provider audit history for the run interval. GitHub organization audit logs and GitLab project audit events must expose an exhausted pagination boundary and are accepted only within the documented 180-day retention window; missing audit access, rate limiting, or an unstable high-water mark fails closed without custody.
