...
| `--keep-local` | `bool` | `false` | With `--recover`: keep the current local head; never touches the worktree, and where the gate branch still names a different head, points it at the kept head |
