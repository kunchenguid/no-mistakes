...
Historical unpublished recovery requires complete, target-bound provider audit history for the run interval. GitHub organization audit logs and GitLab project audit events must expose an authoritative HTTP server-date cutoff and an explicit empty-page pagination terminator; missing audit access, rate limiting, truncated history, or an unstable cutoff fails closed without custody. Legacy rows with missing submission-time request lineage are reconciled only by that complete provider proof and an atomic target-ledger update.
