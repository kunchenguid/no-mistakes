...
When starting a new run, `axi run` refuses both the repository default branch and a separately configured PR base branch, and refuses uncommitted working trees, with actionable errors instead of auto-branching or auto-committing.
...
If that PR later falls behind its intended PR base branch or hits a merge conflict, do not run `axi run`, `rerun`, or a manual rebase while the CI monitor is still running.
