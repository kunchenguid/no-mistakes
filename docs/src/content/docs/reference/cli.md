...
When the CI step is monitoring an open PR and checks are green, `axi run` exits successfully with `outcome: checks-passed` instead of waiting for a human merge. A generic empty check list is not ready. When the trusted default-branch config declares [`no_ci: true`](/no-mistakes/reference/repo-config/#no_ci), the CI step and its forge queries are omitted and the pipeline returns terminal `outcome: passed` after the PR step.
