...
`passed-with-override` is a completed run whose CI or Test approval gate was approved by a human while the live condition was still unresolved (still-failing CI checks, or a failing configured `commands.test`); it stays a success but reads distinctly from a genuinely green `passed`, and its `help` names the failure the operator approved past. A Test override also rides the PR attestation so the required check can see it.
