...
Concurrency coverage deterministically runs two simultaneous repositories with separate simulated `git`/`gh` profiles and asserts that executable, login, credential configuration, and commit identity do not cross. It does not exercise two real authenticated GitHub accounts; operators should treat that as an integration-coverage limitation, not evidence about the isolation contract itself.
