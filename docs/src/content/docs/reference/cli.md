...
Migration does not move a file or Git ref, change any head/generation/outcome, clear the failed error, alter intent, timestamps, findings or rounds, or mark a historical run passed. The fingerprint and provenance are one transaction: interruption leaves both unchanged, and repetition after success reports `repository_rename_already_accepted`. AXI returns `next_action.code: rerun_pipeline` and `no-mistakes rerun`; that starts a new validation from the preserved source and lets ordinary PR discovery, review, tests, publication, and CI produce a new result. It is not approval from existing external checks.
