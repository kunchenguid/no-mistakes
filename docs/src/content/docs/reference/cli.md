...
Cached home and status rendering performs no network read and labels the remote observation `pipeline_push`; explicit synchronization checks, apply, and [repository-rename acceptance](#repository-rename-continuation) can report `live` freshness.
...
| `--accept-repository-rename` | `string` | (none) | Supply the previous credential-free GitHub target for [verified repository-rename continuation](#repository-rename-continuation) |
...
### Repository rename continuation

A repository rename is the one explicit changed-target migration. If the branch is still behind, first complete the ordinary offered synchronization while the old recorded target remains configured. Then use [`no-mistakes init`](#no-mistakes-init) to record the canonical GitHub push-target URL, and pass the credential-free previous target to `--accept-repository-rename` once the clean local, recorded, and pushed heads are exactly equal. The init reference owns how to refresh upstream and fork targets. Acceptance is explicit, does not prompt, and cannot be combined with synchronization, recovery, or archive-binding modes (`--check`, `--yes`, `--recover`, `--keep-local`, or `--bind-archive-ref`). The command requires a terminal run with unchanged target kind and branch ref, no active run on that repository branch, one unambiguous clean checkout, the supplied target's digest equal to the persisted old binding, the current remote branch still at the exact pushed SHA, and authenticated same-host API reads of both names returning the same positive immutable repository ID and the current canonical full name. It refuses missing or malformed identity, API failure, another host, a different repository or fork, changed/dirty/divergent source, and any concurrent ownership or binding change detected before the transaction.

Successful migration changes only the selected terminal run's target fingerprint and atomically records rename provenance bound to that exact run, repository, branch, status, pushed head, target kind, ref, and push generation. Earlier runs retain their original fingerprints. This supports the existing rerun history where an earlier failed run preserved unpublished fixes and a later run successfully pushed those fixes before failing: selection still requires the exact gate branch to equal the later pushed head and Git ancestry to prove containment. Cached inspection and subsequent live checks can use the recorded migration push to bridge the old and current fingerprints only for that lineage. A subsequent push must contain the recorded migration head, which must itself contain the earlier run's preserved head; the earlier run must precede the migration run. Changed or missing provenance, unrelated or uncontained history, and active branch ownership still refuse. This is not a repository-wide alias or a rewrite of historical bindings.

Migration does not move a file or Git ref, change any head/generation/outcome, clear the failed error, alter intent, timestamps, findings or rounds, or mark a historical run passed. The fingerprint and provenance commit together or neither commits. If the final state check refuses after the transaction, AXI reports `safety: blocked_repository_rename_postcondition`, retains the actual `changed` value (which can be `true`), and offers no continuation. Repeating acceptance rechecks the current identity, source, destination, and ownership guards; an unchanged accepted binding reports `repository_rename_already_accepted` and `changed: false` when those guards pass.

Successful acceptance returns exit code `0`, `next_action.code: rerun_pipeline`, and `no-mistakes rerun`; that starts a new validation from the preserved source and lets ordinary PR discovery, review, tests, publication, and CI produce a new result. It is not approval from existing external checks.
...
| `--accept-repository-rename` | `string` | (none) | Use the [repository-rename continuation](#repository-rename-continuation) documented for AXI |

Without `--yes`, apply prints the exact full-SHA plan and requires TTY confirmation; `--recover` prompts the same way before returning custody. Archive binding is itself explicit, does not prompt, and cannot be combined with synchronization, recovery, or `--yes`. For repository-rename acceptance, follow the [shared continuation contract](#repository-rename-continuation).
