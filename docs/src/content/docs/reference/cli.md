...
After custody has been returned for a cancelled run, a complete replacement may intentionally start from an earlier base and omit the cancelled candidate. When the replacement keeps the exact authoritative intent, `axi run` verifies the cancelled run, registered repository, checked-out branch, returned-custody gate binding, and preserved terminal head before atomically retargeting only that stale gate ref. The cancelled gate head and the one permitted replacement head remain under run-specific refs, so a crash or retry converges on the same replacement; any active run, identity mismatch, ambiguous checkout, independently moved gate, or second replacement refuses without changing the gate branch.
...
If that cancelled candidate is later completely superseded after custody return, run the ordinary `no-mistakes axi run --intent "<the same authoritative task>"` from the clean replacement branch. The run command performs the identity-bound stale-gate retirement automatically; do not force-push, merge the obsolete commits, or hand-edit gate refs.
