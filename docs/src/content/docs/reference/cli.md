...
Review gates also include a durable `review_cycle` object with the full review round, fixer executions used, total limit, remaining count, `next_action`, and `allowed_actions`. After the one fixer execution, AXI omits the Fix command and lists only `approve` and `skip`. Schema/output retries are internal attempts within the displayed review round and do not increment this cycle.
...
| `--bind-archive-ref` | `string` | (none)  | Bind one existing `refs/heads/archive/*` commit as exact evidence for lossless recovery of both histories; never creates or moves a Git ref |
...
Under `--recover`, a clean worktree may fast-forward to the preserved pipeline head, adopt a preserved head proven to carry every local change, or receive one ordinary merge commit whose exact local and preserved heads are its parents. The merge path does not force-push, reset, delete a ref, move the gate branch, or select one history over the other.
...
A run that goes terminal (cancelled, failed, or completed without a push stage) after moving the pipeline head leaves the branch `pipeline_owned`. Status offers `next_action.code: recover_custody` only when the exact preserved head is locally reachable or can be imported from its run-specific gate anchor into a clean worktree. The same classification and exact objects are revalidated at the recovery boundary.
...
Without positive evidence that the submitted head stayed unchanged, custody is not guessed away. Conflicting evidence and import cases with a dirty worktree retain manual-reconciliation guidance instead of being mislabeled as a missing preserved head. When a verified recorded head is absent from both the invoking worktree and an accessible local gate, with compatible recovery refs, status reports `safety: blocked_recover_preserved_head_missing` and offers `no-mistakes axi sync --recover --keep-local`: that flag explicitly keeps the current local head and discards the unavailable pipeline commits.
...
A diverged worktree is adopted without a merge only when the preserved head provably carries every local change, using an executable three-way merge whose result is exactly the preserved head's tree. This covers a pipeline rebase or squash without losing the original local head, which is anchored under `refs/no-mistakes/recover-local/<run>`.
Otherwise, related clean histories are joined by one ordinary merge commit. The exact local head is its first parent and the exact terminal pipeline head is its second parent, so unique local commits and pipeline fixes remain reachable and externally auditable. Git computes the merged tree before the branch moves; the branch update is an atomic compare-and-swap, and the worktree update refuses to overwrite concurrent changes.
If that tree has a semantic conflict, recovery leaves the branch and worktree untouched, keeps both heads anchored, and returns exactly one `next_action`: `git merge --no-ff refs/no-mistakes/recover/<run>`. Resolve and commit that merge once, then rerun `axi sync --recover` to stamp custody returned. Recovery never guesses which side's meaning should win.
Terminalization pins the verified unpublished pipeline head under the run-specific recovery ref, so recovery does not require the gate branch itself to advance. The gate branch is not moved by default divergent recovery; the new merge commit descends from the preserved gate head and is ready for a fresh pipeline run.
The containment proof never uses patch identity, which discards hunk locations and whitespace and cannot prove that local content survived.
A dirty worktree refuses with an exact cleanup action.
...
A bound archive supplies exact evidence for the same lossless divergent-history recovery. First create the archive ref through a trusted preservation workflow, then bind that already-existing ref with `--bind-archive-ref`. Binding writes only an append-only recovery-archive record; it never creates, moves, fetches, or deletes a Git ref. The record snapshots the repository, run, branch, exact required local head, exact preserved terminal head, archive ref, and creation time.

Status accepts that evidence only when exactly one record matches the selected terminal run, the clean branch remains at the exact required head, the raw archive ref is a non-symbolic commit at the preserved head, and the gate's run-specific recovery ref independently pins the same commit. The gate branch may be at the required or preserved head. Every field and ref is revalidated before recovery.

A verified archive plan reports `recovery.integration: merge_histories` and offers exactly `no-mistakes axi sync --recover`. Recovery creates the same two-parent merge commit, leaves the archive and gate branch unchanged, and preserves both exact histories. A semantic conflict follows the same one-command manual decision path. `--recover --keep-local` remains an explicit operator choice to discard the unpublished pipeline result, not the default archive action.

For the alternative validation path and its refusal conditions, see [`no-mistakes rerun`](#no-mistakes-rerun).
