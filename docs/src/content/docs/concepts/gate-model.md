...
9. The CI step keeps watching the open PR until it is merged, closed, or its configured idle timeout elapses with no intended-PR-base movement, and can auto-fix failures or merge conflicts when supported.
...
can also end early after `rebase` if the branch has no diff against the intended
PR base (`pr.base_branch`, or the repository default when unset), marking the remaining steps as skipped.
