...
## Run-wide routing and durable history

The daemon owns the routing state for each run.
Before a run starts, it merges global configuration with the repository configuration and validates the resolved routing contract; an invalid contract fails the run before any step launches.
Repository `routes` overrides, like `commands`, are read only from the trusted default branch, never from the pushed SHA.

Each run gets one run-wide provider-circuit state, shared by every routed step, and every circuit starts closed.
A classified operational failure, such as quota, outage, overload, auth, or a missing executable, opens that candidate's provider domain for the rest of the run, and later candidates in an open domain are skipped.
The wizard's standalone suggestion sessions use their own fresh circuits and never share a run's circuit state.
See the [default profile and route tables](/no-mistakes/reference/routing/#default-profiles) for the routing contract itself.

Every routed invocation appends durable, secret-free attempt records to SQLite: the candidate's profile, tier, index, runner, model, and effort, then one terminal fact with the outcome, any classified failure domain, the duration, and token counts.
The daemon reconstructs the review step's routing projection from those tables on every snapshot it serves, so a TUI or AXI client that attaches after a detach, disconnect, or daemon restart still sees the full candidate, circuit-skip, and lineage history without parsing logs.
...
2. Waits up to 30 seconds for it to finish; on timeout it logs a warning and continues
...
- Closes stale standalone utility invocation scopes, such as wizard suggestion sessions whose owning process died, so their routed attempts are not reported as still active
