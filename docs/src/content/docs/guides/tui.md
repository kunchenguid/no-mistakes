...
When Document performs a [combined housekeeping pass](/no-mistakes/reference/global-config/#stage_effort), its row is labeled `Document + Lint housekeeping`; its duration is the shared documentation-and-lint agent invocation, while Lint shows only its cached-result handoff duration.
