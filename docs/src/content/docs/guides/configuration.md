...
- `commands`, `ignore_patterns`, `document.instructions`, `review.path_instructions`, `allow_repo_commands`, `disable_project_settings`, and `test_command_sufficient` are repo-only fields. By default, `commands` and `agent` are read from the trusted default branch; a trusted `allow_repo_commands: true` opt-in instead honors their pushed-branch values. The other gate-control fields, including `review.path_instructions` and the repo `ci` overlay, always come from the trusted default branch; `test_command_sufficient` is dropped under that opt-in because it cannot certify a pushed command. See the [Repo Config Reference](/no-mistakes/reference/repo-config/) security note.
