...
- `commands`, `ignore_patterns`, `document.instructions`, `allow_repo_commands`, and `disable_project_settings` are repo-only fields. `commands` are snapshotted from the exact submitted candidate commit. By default, `agent` comes from the trusted default branch; a trusted `allow_repo_commands: true` compatibility opt-in honors candidate agent selection. The other gate-control fields always come from the trusted default branch. See the [Repo Config Reference](/no-mistakes/reference/repo-config/) security note.
