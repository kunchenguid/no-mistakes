...
Explicit `commands.build`, `commands.test`, and `commands.lint` give you deterministic local baseline behavior, while leaving any empty asks the configured agent to fill the gap: empty `commands.build` has the agent discover and run the smallest relevant build or compile commands, empty `commands.test` has the agent select the smallest relevant tests under the targeted-validation contract (broad regression stays in remote CI), and empty `commands.lint` folds lint into the document step's combined housekeeping pass.
