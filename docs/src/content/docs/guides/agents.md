...
Before the pipeline starts, no-mistakes validates the configured Pi model inside the run's worktree, using the resolution semantics Pi applies to each source. An explicit model (`agent_config.pi` or `agent_args_override`) is resolved by Pi's own CLI resolver - ids, case-insensitive substrings, display names, aliases, ambiguity handling - through a throwaway Pi session, so project packages and extensions that register models are part of the resolution - unless trusted [`disable_project_settings`](/no-mistakes/reference/repo-config/#disable_project_settings) suppresses them, in which case the probe runs with the same suppression as the later agent turn. That probe runs against Pi's offline catalogue first so a cached answer needs no network; an offline miss is re-probed through Pi's real online resolution, and setup fails only when pi's model catalogue is reachable and pi still rejects the model there. A probe killed by its deadline or one that cannot start is inconclusive evidence rather than a verdict, so it falls through the same way. When the catalogue cannot be reached, catalogue verification was not possible: a warning names the model and setup continues, because an absent verification result is not a bad model result. A Pi settings default (`defaultProvider`/`defaultModel`, with project `.pi/settings.json` overriding the global agent settings per key) follows Pi's startup path instead: Pi resolves it by exact provider and model id and silently falls back to another model when the settings file cannot be loaded, the project copy is untrusted or of undetermined trust, a key is missing, or the pair is absent from Pi's catalogue. A project copy whose trust is undetermined draws an inert-copy warning naming the file regardless of the catalogue outcome. no-mistakes checks that pair against Pi's cached offline `pi --list-models` listing only, so a miss there warns that the online catalogue was not checked and Pi will refresh it at startup before choosing a fallback, instead of claiming Pi will fall back. A catalogue listing that itself cannot be produced is reported as unverifiable rather than as a missing model. Each of those cases only logs a warning naming the settings file - setup never fails on a settings value Pi itself would not fail on.
