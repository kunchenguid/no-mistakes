...
A `recover_custody` action is ordinary `no-mistakes axi sync --recover` to take a still-available preserved head, or `no-mistakes axi sync --recover --keep-local` when that head is unavailable and you are discarding the missing commits, when a bound archive preserves divergent later work while custody returns at the reported required head, or when two bound archives preserve sibling pipeline heads of a run that ended without a verified final head and custody returns without selecting either; never substitute one action for the other. See [`no-mistakes rerun`](/no-mistakes/reference/cli/#no-mistakes-rerun) for the alternative validation path and its refusal conditions.
A run that ended without a verified final head, because a fix commit landed as a sibling of the recorded head, reports `safety: blocked_recover_manual_reconciliation` and offers no recovery: an unbound sibling pair looks the same as a run that lost its worker. Keep each sibling at its own `refs/heads/archive/*` ref, then run `no-mistakes axi sync --bind-archive-ref <ref>` once for each ref. The first bind exits `0` with `recovery.proof: awaiting_sibling` and `next_action.code: bind_sibling_archive`; the second reports `recovery.proof: verified` and `next_action.code: recover_custody`. Then run that reported command: custody returns at `recovery.required_head`, neither sibling is selected, and both archives stay for you to reapply in one line.
