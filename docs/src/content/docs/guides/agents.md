...
When `next_action.code` is `recover_remote_rewritten` - the live remote no longer equals the persisted pipeline push binding because something outside this pipeline force-updated it - run `no-mistakes axi sync --recover` to correct the binding to the freshly verified live head; it never moves the worktree, the branch, or the gate, it anchors the superseded pipeline head under `refs/no-mistakes/recover/<run>` first, and it reports `recovery: push_binding_corrected` rather than a custody return - never hand-edit local state instead.
