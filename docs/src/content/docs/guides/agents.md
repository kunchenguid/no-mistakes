...
If that PR later falls behind its pipeline base or hits a merge conflict - commonly because another PR merged first - the agent runs no command and must never hand-rebase.
...
In task-first mode, if the repo is on the repository default or configured pipeline base, the skill tells the agent to create a feature branch before committing because both policy branches are protected sources. When they differ, a feature created from the repository default must start at the fetched pipeline base instead of carrying default-only history.
