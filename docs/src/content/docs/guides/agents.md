...
With the default `session_reuse: true`, Claude, Codex, Grok, Pi, and Antigravity keep one fixer session per run. ACP targets through acpx run cold because acpx cannot atomically bind the complete provider implementation and configuration to the processes receiving a stored session identity and prompt.
