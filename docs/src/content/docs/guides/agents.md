...
The same preamble bounds the read side: agents may read outside the worktree and run read-only commands, but must not run filesystem-wide searches such as `find /` or `mdfind /`, and must not hunt the host filesystem for an installed tool. When a needed tool is not on `PATH` and no repository-local path is supplied, agents may obtain, install, or build it inside the disposable worktree, but must not search the host for it or install it system-wide or globally. Only if no workspace-local route works do they report the missing tool and blocked work; see the [Test step](/no-mistakes/reference/pipeline-steps/#test) for live-scenario reporting.
