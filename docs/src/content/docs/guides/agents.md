...
Trusted image-publication consent and PR rendering are owned by the [repo](/no-mistakes/reference/repo-config/#testevidence) and [global](/no-mistakes/reference/global-config/#testevidence) evidence references.
...
That testing invocation is expected to leave only intentional source or test-file changes in the worktree, while preserving requested evidence files under the dedicated temporary evidence directory.
