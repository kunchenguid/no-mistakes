...
Before the pipeline starts, no-mistakes asks Pi itself to resolve an explicit model, or the `settings.json` default when no model is configured, inside the run's worktree: the probe runs a throwaway Pi session there, so project `.pi/settings.json` (which overrides the global agent settings) and project packages/extensions are part of the resolution and Pi's own pattern semantics (ids, case-insensitive substrings, display names, aliases, ambiguity handling) apply unchanged. A project default is only allowed to fail setup when Pi would honor it: a recorded Pi trust decision for the worktree, an explicit `--approve`, or `defaultProjectTrust: always` in the global agent settings. When Pi's trust for the worktree is undecided, a failing project default only logs a warning that the setting may be inert, and a project file Pi does not trust or cannot load is ignored, because setup must not fail on a value Pi itself would never act on. When Pi's resolver rejects the selection, setup fails with the setting source and Pi's error instead of running with guessed capabilities.
