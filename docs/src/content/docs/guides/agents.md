...
When scheduled CI checks are green but the PR is still open, `axi run` and `axi respond` return `outcome: checks-passed` with a help line pointing at the PR instead of waiting for a human merge. A trusted default-branch [`no_ci: true`](/no-mistakes/reference/repo-config/#no_ci) declaration instead omits the CI step before constructing its forge client or monitor, so the run returns terminal `outcome: passed` immediately after PR creation with no CI polling or background monitor. An empty check result without that declaration is not ready; see the [CI step reference](/no-mistakes/reference/pipeline-steps/#ci) for the scheduling and readiness rules.
...
When CI is scheduled, its monitor stays live in the background after checks pass, and when it sees an actual conflict it rebases onto the base, resolves it, restarts validation at Review, and re-pushes the branch through Push, so no agent or user action is needed.
