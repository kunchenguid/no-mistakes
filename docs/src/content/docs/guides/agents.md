...
When `next_action.code` is `rerun`, run `no-mistakes rerun`: it starts from the retained gate branch and replays the logical change rather than recovering the recorded commit.
