...
With the default `session_reuse: true`, Claude, Codex, Grok, Pi, Antigravity, and ACP targets through acpx keep one fixer session per run. An ACP fixer's first turn remains the existing one-shot `exec --file -`; no-mistakes captures the identity it reports without another ACP target launch. Later turns hash the effective executable, environment, target, model, raw override and its identifiable file artifacts, and named acpx configuration locally before exposing the stored ID to `sessions new --resume-session`, then recheck that binding between session operations. A changed or unverifiable stored binding and any other stored ACP setup failure discard that identity and run exactly one cold `exec`; any error after a prompt may have been delivered fails the turn without replay. If acpx accepts the requested ID during setup but the installed target reports a replacement during the prompt, that observed replacement becomes the durable identity.
...
The [`session_reuse` field reference](/no-mistakes/reference/global-config/#session_reuse) owns the exact reuse, fallback, local-isolation, privacy, and restart-recovery semantics.
...
For review-fixer reuse, the first turn stays on acpx's one-shot `exec --file -` path and captures the ACP identity it reports, with no separate session setup or additional ACP target launch. A local `acpx config show` probe fingerprints the effective acpx binary, target, raw override, and model. Later turns require that fingerprint to match before `sessions new --resume-session` and recheck it before prompt and bounded close; raw commands also require an unambiguous executable and fingerprint every argument that names a file. Stored-session configuration or setup failures before prompt delivery fall back to one fresh `exec`; once either cold or resumed prompt execution starts, an error fails without replay or agent fallback. Bridge names and provider fingerprints are opaque local isolation data derived from trusted run scope and effective serving configuration; raw commands and ACP identities also stay local.
