...
Before the pipeline starts, no-mistakes validates the configured Pi model inside the run's worktree, using the resolution semantics Pi applies to each source. An explicit model (`agent_config.pi` or `agent_args_override`) is resolved by Pi's own CLI resolver - ids, case-insensitive substrings, display names, aliases, ambiguity handling - through a throwaway Pi session in the worktree, so project packages and extensions that register models are part of the resolution; when Pi rejects the pattern, setup fails with Pi's error. A Pi settings default (`defaultProvider`/`defaultModel`, with project `.pi/settings.json` overriding the global agent settings per key) follows Pi's startup path instead: Pi resolves it by exact provider and model id and silently falls back to another model when the settings file cannot be loaded, the project copy is untrusted, a key is missing, or the pair is absent from `pi --list-models`. Each of those cases only logs a warning naming the settings file - setup never fails on a settings value Pi itself would not fail on.
