...
When a repository opts into `disable_project_settings`, the opencode serve argv carries `--no-project-instructions` and `--pure` as managed flags appended after any operator overrides so yargs last-wins enforces them. `--no-project-instructions` disables project instructions (AGENTS.md/CLAUDE.md), project config (`.opencode/`), and project skill discovery; `--pure` disables external/project plugins. An older opencode that does not recognize `--no-project-instructions` is rejected with a concrete diagnostic naming the required capability and upgrade path, rather than silently running with project instructions loaded. Explicit operator model arguments (e.g. `--model`) from `agent_args_override` are preserved.
