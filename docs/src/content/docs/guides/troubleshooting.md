...
This section applies only when CI was scheduled. A trusted default-branch [`no_ci: true` declaration](/no-mistakes/reference/repo-config/#no_ci) schedules an eight-step pipeline with no CI step, forge client, polling loop, or zero-check query; after PR delivery, that run becomes terminal `passed` instead of showing a CI panel or background monitor. A feature-branch declaration cannot create this exemption.
...
Once the CI monitor reports readiness and the PR is mergeable, the CI panel shows `✓ Checks passed` and the terminal title switches to `Checks passed`, so you can tell when to go merge the PR; the signal clears automatically if checks start re-running or a new failure appears. An empty forge response is not ready. The [CI step reference](/no-mistakes/reference/pipeline-steps/#ci) owns the exact readiness and signal-clearing rules, including GitHub Actions runs that do not appear in the PR check rollup.

When CI is scheduled, how long the monitor runs is controlled by `ci_timeout` in `~/.no-mistakes/config.yaml`, an idle timeout that re-arms whenever the upstream default branch advances; the [`ci_timeout` field reference](/no-mistakes/reference/global-config/#ci_timeout) owns the default, the `unlimited` keyword and its aliases, and the exact re-arm semantics.
