...
### Rebase pauses because the branch carries unpushed PR-base commits

This means the feature branch was created from a local copy of the intended PR base that is ahead of its upstream ref, so its history includes commits that exist only on your local base branch. The intended base is `pr.base_branch` when configured and otherwise the repository default branch.
...
Monitoring while the PR remains open - even after checks are currently healthy - is intended behavior, because a later intended-PR-base update can make the PR conflict or rerun CI.
...
How long the monitor runs is controlled by `ci_timeout` in `~/.no-mistakes/config.yaml`, an idle timeout that re-arms whenever the upstream intended PR base advances (`pr.base_branch` when configured, otherwise the repository default); the [`ci_timeout` field reference](/no-mistakes/reference/global-config/#ci_timeout) owns the default, the `unlimited` keyword and its aliases, and the exact re-arm semantics.
