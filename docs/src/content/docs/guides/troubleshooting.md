...
Requested test evidence may still be written under the managed temporary `no-mistakes-evidence` directory.
Published screenshot pixels, when a trusted repository opts in, stay inside the disposable worktree under the fixed generated evidence namespace.
