...
- The provider CLI reports that it is not authenticated; on GitHub, a timed-out or interrupted `gh auth status` is reported separately from auth failure. GitHub's `ssh.github.com:443` SSH transport remains the canonical `github.com` forge identity and does not require a separate `gh auth login`.
