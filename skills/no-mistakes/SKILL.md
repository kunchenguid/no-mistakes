...
- A non-terminal run object may include `awaiting_agent: parked <duration>` immediately after `status`; that means the run is parked at a gate. Run `axi respond` only from that run's own worktree or branch, and follow the response's `help`.
