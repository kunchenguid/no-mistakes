...
When `next_action.code` is `recover_custody`, the exact recorded head is durably preserved for guarded recovery: run `no-mistakes axi sync --recover` to return custody and take it.
...
When `next_action.code` is `rerun`, the recorded head lacks recovery's exact authority: run `no-mistakes rerun`. It starts from the retained gate branch and replays the logical change rather than recovering the recorded commit.
