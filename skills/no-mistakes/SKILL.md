...
Run only its exact identity-bound `no-mistakes axi sync --release-unavailable --run <id>` command: it refuses unless the recorded head remains unavailable and the clean local branch exactly matches its fresh configured remote, then anchors every reachable local, remote, and gate head before releasing custody.
