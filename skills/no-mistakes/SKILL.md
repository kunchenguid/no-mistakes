...
If you choose `no-mistakes rerun` instead, rerun starts from the current ordinary gate branch, not the preserved recovery anchor.
Recovery anchors the preserved commits under `refs/no-mistakes/recover/<run>` before it fast-forwards a clean behind worktree or refuses with explicit choices; `--keep-local` keeps your current head while the preserved commits stay anchored under `refs/no-mistakes/recover/<run>`.
