...
When `next_action.code` is `recover_custody`, run its exact `next_action.command` rather than reconstructing one. The ordinary `no-mistakes axi sync --recover` action returns custody and takes a preserved pipeline head; `no-mistakes rerun` can resume validating that ordinary head instead.
Ordinary recovery takes the head by fast-forward, or by adopting a diverged preserved head proven to carry every local change - the ordinary result of the pipeline rebasing your commits onto a newer base - after anchoring your pre-recovery head under `refs/no-mistakes/recover-local/<run>`.
A reported `no-mistakes axi sync --recover --keep-local` action is narrower: an explicitly bound archive proves divergent later work remains preserved, while recovery keeps the branch at the exact reported required head and never selects, merges, or replays the archive. Do not substitute plain `--recover` or `rerun` for that action.
