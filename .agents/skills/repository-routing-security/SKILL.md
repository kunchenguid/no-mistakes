...
- `repos.upstream_url` is the registered repository used for ordinary PR base routing; `repos.fork_url` is an optional GitHub fork push target. Explicit `axi run --existing-pr <URL>` is the exception, and it is remembered per repository+branch (`branch_pr_targets`), so later unflagged, push-hook and rerun launches publish to the same pull request until it is replaced or retired: `internal/pipeline/steps/existing_pr.go` owns fail-closed identity validation, and the [CLI reference](../../../docs/src/content/docs/reference/cli.md#explicit-existing-upstream-pr) owns the supported lifecycle. Never inherit its URL into a run without its repository constraint (`inheritablePRURL` owns that boundary); the association itself is carried by `runs.existing_pr_url`, never by the legacy PR-URL inheritance.
