...
- Eval replay identity uses `agentcfg.ServedMatchesRequested`: adapters such as Pi report a bare model plus a separate provider, so `xai/grok-4.6` matches served `grok-4.6`/`xai`. Bare string equality is the fallback; a different model or provider still fails. Do not work around this by requesting the bare model name.
