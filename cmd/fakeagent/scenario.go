...
const reviewPromptMarker = `Invoke the user-installed "code-review" skill`
...
// and the post-fix rereview), filtered by the prompt's excluded paths with
...
func reviewCoverageForPrompt(wd, prompt string) ([]string, error) {
	...
	var ignore []string
	if raw := promptContextValue(prompt, "excluded paths"); raw != "" && raw != "none" {
