...
- On ` + "`--action fix`" + `, always name the findings the user chose to fix with
  ...
  counted as answered; a fix response with no ` + "`--findings`" + ` resolves nothing,
  so the same gate re-parks on the same findings and will keep doing so until
  you select, approve, or skip them. That is deliberate - a finding the user
  never answered must not disappear - so read a repeat of the same gate as
  "nothing was selected yet", not as the pipeline being stuck.
