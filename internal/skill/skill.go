...
When ` + "`next_action.code`" + ` is ` + "`recover_custody`" + `, a terminal run left unpublished pipeline commits preserved in the local gate: run the reported command. That is ` + "`no-mistakes axi sync --recover`" + ` to take the preserved head, or ` + "`no-mistakes axi sync --recover --keep-local`" + ` when the preserved head is unavailable and you are explicitly keeping the current local head. ` + "`no-mistakes rerun`" + ` resumes validating a still-available preserved head instead.
...
A dirty worktree, or divergence that cannot be proven contained, makes the recovery refuse with explicit choices; ` + "`--keep-local`" + ` keeps your current head while the preserved commits stay anchored under ` + "`refs/no-mistakes/recover/<run>`" + `. The same flag is the recovery when the preserved head is missing: it returns custody at the current local head without requiring that object.
