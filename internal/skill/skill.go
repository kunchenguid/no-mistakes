...
For a known GitHub owner/repository rename, first synchronize while the old target remains configured if the local branch is behind. After updating ` + "`origin`" + ` and running ` + "`no-mistakes init`" + ` to record the canonical name, ` + "`no-mistakes axi sync --accept-repository-rename <previous-credential-free-target>`" + ` is the only target-change migration. It requires a terminal run, exact clean local/pushed/live heads, unchanged kind/ref, no active same-branch run, and authenticated same-host API proof that both names have one positive immutable repository ID and the current canonical full name. It changes only the old push fingerprint, leaves the historical run failed, and returns an exact ` + "`rerun_pipeline`" + ` action for new validation. Unknown identity, API failure, a different repository/fork/host, or changed ownership/source remains blocked.
...
no-mistakes axi sync --accept-repository-rename <previous-target>  # migrate only a verified same-ID GitHub rename binding
