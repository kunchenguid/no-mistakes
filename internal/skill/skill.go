...
If any copy of that head still exists, the settlement anchors it at ` + "`refs/no-mistakes/recover-stranded/<run>`" + ` before moving the gate branch off it - but only in whichever repository still holds the object, your worktree or the local gate under ` + "`NM_HOME`" + ` or both, so look in both rather than assuming your own; if no store still has it, nothing is anchored because nothing was left to anchor and the settlement took nothing from you.
That anchor is NOT ` + "`refs/no-mistakes/recover/<run>`" + `, which in this state is the conflicting evidence and can name an entirely different commit.
When ` + "`next_action.code`" + ` is ` + "`complete_custody_return`" + `, an earlier recovery already applied its Git changes and only the custody record is missing: re-run the exact command it names - ` + "`no-mistakes axi sync --recover`" + ` or ` + "`no-mistakes axi sync --recover --keep-local`" + `, whichever it carries - to complete the record.
That code is deliberately neither ` + "`recover_custody`" + ` nor ` + "`return_custody_keep_local`" + `: each of those is a claim about the record, and neither is being made here. Re-running is safe because every Git step it repeats is idempotent once applied.
