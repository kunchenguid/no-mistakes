...
For a known GitHub owner/repository rename, first complete the offered synchronization while the old target remains configured if the local branch is behind. Refresh the canonical push target with ` + "`no-mistakes init`" + `; for a fork, use ` + "`--fork-url`" + ` and keep ` + "`origin`" + ` at the parent, following the [init reference](https://kunchenguid.github.io/no-mistakes/reference/cli/#no-mistakes-init). Then ` + "`no-mistakes axi sync --accept-repository-rename <previous-credential-free-target>`" + ` is the only target-change migration. It requires a terminal run, exact clean local/pushed/live heads, unchanged target kind/ref, no active same-branch run, and authenticated same-host API proof that both names have one positive immutable repository ID and the current canonical full name. It atomically migrates the selected terminal run's fingerprint and records exact rename provenance for a contained rerun lineage; earlier fingerprints and all validation history remain unchanged. A failed run remains failed. Follow the exact ` + "`rerun_pipeline`" + ` action after successful acceptance to start new validation. Unknown identity, API failure, a different repository/fork/host, or changed ownership/source remains blocked. The [continuation reference](https://kunchenguid.github.io/no-mistakes/reference/cli/#repository-rename-continuation) owns the provenance and refusal details, including a final state check that can block after migration commits: honor the reported state even when ` + "`changed: true`" + `.
...
no-mistakes axi sync --accept-repository-rename <previous-target>  # accept a verified same-ID GitHub rename with exact provenance
