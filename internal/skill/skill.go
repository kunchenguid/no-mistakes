...
When ` + "`next_action.code`" + ` is ` + "`verify_preserved_head`" + `, run ` + "`no-mistakes axi sync --verify-preserved-head`" + ` first. It anchors one exact terminal pipeline commit and records verification without moving the branch or worktree; then follow the newly reported next action.
When ` + "`next_action.code`" + ` is ` + "`recover_custody`" + `, a terminal run left verified unpublished pipeline commits preserved in the local gate: run ` + "`no-mistakes axi sync --recover`" + ` to return custody and take the preserved head, or ` + "`no-mistakes rerun`" + ` to resume validating it instead.
