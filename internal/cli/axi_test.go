...
func TestRerunParamsIncludeSkipSteps(t *testing.T) {
	params := rerunParams("repo-1", "feature/x", []types.StepName{types.StepReview}, "user goal", "develop", true)
	...
	if !params.RequirePonytail {
		t.Fatal("RequirePonytail = false, want true")
	}
}
