...
func attachRun(ctx context.Context, w io.Writer, runID string, rootDefault bool, autoYes bool, skipSteps []types.StepName) error {
	...
	if runID != "" {
		...
		// for the wizard (detached HEAD, or a protected default/PR-base
		// branch with pending changes) — pipelines for the user's current
		// situation don't match any existing run.
