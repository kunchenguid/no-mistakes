...
func newAxiCmd() *cobra.Command {
	...
	cmd.AddCommand(newAxiWizardCmd())
	cmd.AddCommand(newAxiCanaryCmd())
	return cmd
}
...
func runAxiHome(cmd *cobra.Command) (string, error) {
	...
	help = append(help, "The calling agent drives AXI gates but does not replace the configured pipeline agent; run `no-mistakes doctor` if no configured routing runner is available")
