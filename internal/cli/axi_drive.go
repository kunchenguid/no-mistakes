...
func newAxiRunCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return trackAxiSurface("axi-run", "/axi/run", telemetry.Fields{
				...
				return runAxiRun(cmd, autoYes, skipSteps, intent, draftUntilReady)
			})
		},
	}
	...
	cmd.Flags().BoolVar(&draftUntilReady, "draft-until-ready", false, "open the pull request as a draft and mark it ready for review only once CI is green")
	return cmd
}

func runAxiRun(cmd *cobra.Command, autoYes bool, skipSteps []types.StepName, intent string, draftUntilReady bool) error {
	ctx := cmd.Context()
	...
	if runID == "" {
		...
		var err error
		runID, err = triggerRun(ctx, env, branch, headSHA, skipSteps, intent, draftUntilReady)
		if err != nil {
...
func triggerRun(ctx context.Context, env *axiEnv, branch, headSHA string, skipSteps []types.StepName, intent string, draftUntilReady bool) (string, error) {
	pushOptions := formatSkipPushOptions(skipSteps)
	if opt := formatIntentPushOption(intent); opt != "" {
		pushOptions = append(pushOptions, opt)
	}
	if opt := formatDraftUntilReadyPushOption(draftUntilReady); opt != "" {
		pushOptions = append(pushOptions, opt)
	}
	...
	if err := env.client.Call(ipc.MethodRerun, rerunParams(env.repo.ID, branch, skipSteps, intent, draftUntilReady), &rr); err != nil {
...
func rerunParams(repoID, branch string, skipSteps []types.StepName, intent string, draftUntilReady bool) *ipc.RerunParams {
	return &ipc.RerunParams{RepoID: repoID, Branch: branch, SkipSteps: skipSteps, Intent: intent, DraftUntilReady: draftUntilReady}
}
