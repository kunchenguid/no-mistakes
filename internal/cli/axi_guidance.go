...
// behind its configured pipeline base branch or hits a merge conflict (commonly
// because another PR merged first). The base may be an explicit non-default
// branch; only an unset base_branch uses the repository default. The live CI
// monitor keeps running after checks pass and auto-rebases onto that base,
// resolves the conflict, and re-pushes the branch itself, so the agent runs no
// command and never hand-rebases. `no-mistakes rerun` is only the recovery for
...
const staleMonitorGuidance = "If this PR later falls behind its configured pipeline base branch or hits a merge conflict, the CI monitor rebases onto that base, resolves it, and re-pushes the branch automatically - run no command and never hand-rebase. The pipeline base may be an explicit non-default branch; only an unset `base_branch` uses the repository's default branch. Only when that monitor is no longer running (PR closed, run aborted, idle-timeout, or auto-fix exhausted) recover with `no-mistakes rerun`."
