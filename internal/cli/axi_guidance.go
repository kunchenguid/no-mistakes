...
const branchSyncAgentGuidance = "Before a post-pipeline local commit or fresh run, follow the structured `branch_sync.next_action`. Run `no-mistakes axi sync` only when its code is `sync`; that guarded sync may be a strict fast-forward or a content-equivalent diverged advance that anchors the pre-sync head before moving the branch with reset semantics. Run `no-mistakes axi sync --recover` only when its code is `recover_custody` (a terminal run left unpublished pipeline commits preserved in the local gate). Run `no-mistakes axi sync --recover --keep-local` only when its code is `return_custody_keep_local` (a terminal run whose recorded pipeline head can no longer be verified): it returns custody at your current head and points the gate branch at it where that branch still names a different head. A `complete_custody_return` code separately authorizes whichever of those two recovery commands it names: an earlier recovery already applied its Git changes and only the custody record is missing, so re-running the exact command it names completes the record. A `user_owned` state means cancellation released the branch before changing the submitted head: the exact branch and head are yours, immediately usable, and no sync action is needed. Process blocked or pipeline-owned states instead of improvising reset, stash, merge, rebase, force, or branch replacement."
