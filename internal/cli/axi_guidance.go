...
const branchSyncAgentGuidance = "Before a post-pipeline local commit or fresh run, follow the exact structured `branch_sync.next_action.command`. A `sync` action may be a strict fast-forward or a content-equivalent diverged advance that anchors the pre-sync head before moving the branch with reset semantics. A `recover_custody` action uses `no-mistakes axi sync --recover`: it fast-forwards, adopts a preserved head proven to contain all local work, or creates one ordinary merge commit with the exact clean local and terminal pipeline heads as parents. A semantic conflict stops before branch mutation and reports one exact manual merge action. Use `--recover --keep-local` only when the structured action says so and you explicitly choose the current head over unavailable or unwanted pipeline commits; never substitute one recovery command for the other. A bound archive preserves a divergent later head as exact evidence for the ordinary lossless recovery. Never hand-edit files while state is `pipeline_owned`; recovery returns custody first."
