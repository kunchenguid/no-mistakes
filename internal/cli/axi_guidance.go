...
const branchSyncAgentGuidance = "Before a post-pipeline local commit or fresh run, follow the structured `branch_sync.next_action`. Run `no-mistakes axi sync` only when its code is `sync`; that guarded sync may be a strict fast-forward or a content-equivalent diverged advance that anchors the pre-sync head before moving the branch with reset semantics. Run `no-mistakes axi sync --recover` only when its code is `recover_custody` (a terminal run left unpublished pipeline commits preserved in the local gate) or `recover_remote_rewritten` (the live remote was force-updated outside this pipeline, so the persisted push binding is corrected to the freshly verified live head, reported as `recovery: push_binding_corrected`, after the superseded pipeline head is anchored and without moving the worktree, your branch, or the gate). A `user_owned` state means cancellation released the branch before changing the submitted head: the exact branch and head are yours, immediately usable, and no sync action is needed. Process blocked or pipeline-owned states instead of improvising reset, stash, merge, rebase, force, or branch replacement, and never hand-edit local state to work around a block."
