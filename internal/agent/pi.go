...
func (a *piAgent) runOnce(ctx context.Context, opts RunOpts) (*Result, error) {
	...
	// Bind this exact process to its pi session JSONL so buffered stdout does
	// not read as silence. The watcher credits only unambiguous session
	// advancement; when no binding exists it is nil and stdout/lifecycle
	// evidence alone governs. It snapshots pre-existing session files here,
	// strictly before the process can create its own, so the fresh-session
	// binding cannot race pi's startup. It stops when this invocation's
	// context ends or runOnce returns, whichever comes first.
	watch := startPiSessionWatcher(ctx, a.extraArgs, cmd.Env, opts.CWD, opts.Session, pp.sessionIDString, opts.OnActivity, func(msg string) {
		emitLifecycle(opts, LifecycleEvent{Agent: "pi", Phase: "liveness", Message: msg})
	})
	if watch != nil {
		defer watch.shutdown()
	}
	...
	if err == nil && opts.Session != nil {
		...
		} else if opts.Session.ID != "" && pp.sessionIDString() != opts.Session.ID {
			err = fmt.Errorf("pi did not confirm the requested session")
		} else {
			res.SessionID = pp.sessionIDString()
...
type piParser struct {
	...
	// sessionID is written by the parse goroutine and read by the liveness
	// watcher goroutine (late header confirmation), so it is atomic (stores
	// string; unset means no header parsed yet).
...
func (p *piParser) sessionIDString() string {
	id, _ := p.sessionID.Load().(string)
	return id
}
