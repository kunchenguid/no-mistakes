...
func finalizeGrokResult(result *Result, schema json.RawMessage) (*Result, error) {
	...
	if len(schema) > 0 && (len(result.Output) == 0 || string(result.Output) == "null") {
		return nil, rejectStructuredOutput(errGrokNoStructuredOutput, RejectedOutput{})
	}
	if len(schema) > 0 {
		if err := validateStructuredOutput(result.Output, schema); err != nil {
			return nil, rejectStructuredOutput(fmt.Errorf("grok structured output: %w", err), RejectedOutput{Text: result.Output})
		}
	}
	return result, nil
}
