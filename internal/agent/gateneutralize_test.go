...
// fail-closed contract: under the opt-out, only adapters whose suppression
// contracts are empirically verified neutralize the required behavior
// resources; every other harness reports false and is refused rather than
// launched with project instructions loaded.
...
// closed for an unsupported harness with a clear error, and admits every
// verified adapter.
...
func TestNeutralizesGateInstructions_ThroughProductionWrapping(t *testing.T) {
	...
	if err := EnsureGateNeutralized(allVerified); err != nil {
		t.Errorf("fallback [codex, claude, pi] must pass under opt-out: %v", err)
	}
...
// admitted; a defeating one fails closed - even for verified adapters.
func TestNeutralizesGateInstructions_HonestOnEffectiveOverride(t *testing.T) {
	...
	// Pi: model/thinking selection is unrelated to resources -> admitted.
	if !NeutralizesGateInstructions(optOutAgent(t, types.AgentPi, []string{"--model", "openai-codex/gpt-5.6-sol", "--thinking", "medium"})) {
		t.Error("pi model/thinking overrides must preserve neutralization")
	}
	// Pi: explicit resources and prompt/trust overrides bypass or alter one of
	// the managed isolation controls -> fail closed.
	for _, conflict := range []string{"--extension", "--skill", "--prompt-template", "--theme", "--approve", "--system-prompt", "--append-system-prompt", "@AGENTS.md"} {
		if err := EnsureGateNeutralized(optOutAgent(t, types.AgentPi, []string{conflict})); err == nil {
			t.Errorf("pi override %q must be refused by the gate", conflict)
		}
	}
}
