...
// recoverOnStartup cleans up after a previous daemon crash. Before generic
// stale-run failure and worktree cleanup, it reconciles any exact prepared head
// advance whose gate already reached its immutable candidate. It also kills
// orphaned managed-server subprocesses (opencode, rovodev) and best-effort
// migrates gate bare repos in place so older installs pick up the per-worktree
// hookspath isolation introduced for issue #122 when Git supports
// config --worktree.
