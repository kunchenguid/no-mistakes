...
func TestRunStartRefreshesCloneURLWithoutMutatingRemotes(t *testing.T) {
	...
	runID, err := manager.startRun(context.Background(), repo, "main", head, refreshTestZeroSHA, "test", RunOptions{Intent: "refresh repository URL"})
	if err != nil {
...
func TestRunStartURLRefreshFailuresWarnSafelyAndContinueWithOldRegistration(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			t.Cleanup(manager.Shutdown)
			runID, err := manager.startRun(context.Background(), before, "main", head, refreshTestZeroSHA, "test", RunOptions{Intent: "refresh failure must fail open"})
			if err != nil {
