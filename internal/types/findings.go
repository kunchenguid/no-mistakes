...
type Finding struct {
	...
	// Check is the provider check name a CI finding was derived from, so the
	// fix half can fetch that check's logs by name and a human sees which
	// check is red. Empty on every non-CI finding.
