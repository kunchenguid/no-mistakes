...
type Run struct {
	...
	// ReviewFixConfigJSON is the resolved global-only fixer routing recorded
	// before this run starts. Nil marks a run created before role snapshots.
	ReviewFixConfigJSON    *string
...
const runColumns = `id, repo_id, branch, head_sha, base_sha, worktree_dir, submitted_head_sha, no_mistakes_version, no_mistakes_build_sha, review_approved_head_sha, review_fix_config_json, status, pr_url, pr_state, pr_state_observed_at, ci_ready_at, COALESCE(ci_ready_no_ci, 0), last_pushed_sha, push_target_kind, push_target_fingerprint, push_ref, last_pushed_at, push_generation, COALESCE(push_active, 0), terminal_head_verified_at, custody_returned_at, error, awaiting_agent_since, COALESCE(parked_ms, 0), intent, intent_source, intent_session_id, intent_score, pr_base_branch, created_at, updated_at`

func scanRun(row interface {
	Scan(...any) error
}, r *Run) error {
	return row.Scan(
		&r.ID, &r.RepoID, &r.Branch, &r.HeadSHA, &r.BaseSHA, &r.WorktreeDir, &r.SubmittedHeadSHA, &r.NoMistakesVersion, &r.NoMistakesBuildSHA, &r.ReviewApprovedHeadSHA, &r.ReviewFixConfigJSON, &r.Status,
...
// SetRunReviewFixConfig records the role routing resolved during setup. It is
// written before execution begins so daemon recovery never reinterprets a new
// global override as belonging to an already-running pipeline.
func (d *DB) SetRunReviewFixConfig(id, encoded string) error {
	_, err := d.sql.Exec(`UPDATE runs SET review_fix_config_json = ?, updated_at = ? WHERE id = ?`, encoded, now(), id)
	if err != nil {
		return fmt.Errorf("set run review fixer config: %w", err)
	}
	return nil
}
