...
func (s *DocumentStep) Execute(sctx *pipeline.StepContext) (*pipeline.StepOutcome, error) {
	...
	// lint command is configured and effective efforts are compatible; the
	// lint step then consumes the result
	// instead of paying its own cold agent invocation.
	combinedLint := sctx.Config.CombineHousekeeping()
	// Never leave a previous combined pass available after a split or skip.
