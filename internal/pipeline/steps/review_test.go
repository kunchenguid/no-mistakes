...
func TestReviewStep_FixMode(t *testing.T) {
	...
	ag := &mockAgent{
		...
		runFn: func(ctx context.Context, opts agent.RunOpts) (*agent.Result, error) {
			...
			findings := Findings{Items: nil, Summary: "all clear", RiskLevel: "low", RiskRationale: "all clear", RiskScope: types.FindingsRiskScopeSourceOrExternal}
	...
	ag := &mockAgent{
		...
		runFn: func(_ context.Context, opts agent.RunOpts) (*agent.Result, error) {
			...
			case 3:
				assertTestQualityRulePrompt(t, opts.Prompt)
				assertTestQualityReviewerAction(t, opts.Prompt)
				output, _ := json.Marshal(Findings{Summary: "clean", RiskLevel: "low", RiskRationale: "clean", RiskScope: types.FindingsRiskScopeSourceOrExternal})
				return &agent.Result{Output: output}, nil
			default:
...
func TestReviewStep_ConcurrentHeadResetCannotGainApproval(t *testing.T) {
	...
	ag := &mockAgent{
		...
		runFn: func(_ context.Context, _ agent.RunOpts) (*agent.Result, error) {
			...
			findings, _ := json.Marshal(Findings{Summary: "all clear", RiskLevel: "low", RiskRationale: "all clear", RiskScope: types.FindingsRiskScopeSourceOrExternal})
...
func TestReviewStep_FixMode_FocusedVerificationContract(t *testing.T) {
	...
	ag := &mockAgent{
		...
		runFn: func(ctx context.Context, opts agent.RunOpts) (*agent.Result, error) {
			...
			j, _ := json.Marshal(Findings{Summary: "clean", RiskLevel: "low", RiskRationale: "clean", RiskScope: types.FindingsRiskScopeSourceOrExternal})
			return &agent.Result{Output: j}, nil
		},
	}
...
func TestReviewStep_DurableFixAdequacyContract(t *testing.T) {
	...
	findingsJSON, _ := json.Marshal(Findings{Summary: "clean", RiskLevel: "low", RiskRationale: "clean", RiskScope: types.FindingsRiskScopeSourceOrExternal})
	ag := &mockAgent{
		name: "test",
		runFn: func(ctx context.Context, opts agent.RunOpts) (*agent.Result, error) {
