...
- When changed code integrates with an external executable, service, SDK, configuration source, or environment-dependent runtime, verify the dependency's actual semantics from its authoritative interface or implementation. Trace precedence among explicit flags, environment variables, configuration files, defaults, and fallbacks, and verify the executable and required credentials exist in the environment of the process that actually consumes them. Do not assume that a fallback is exclusive, that the review shell matches a daemon or worker environment, or that another tool's authentication applies.
