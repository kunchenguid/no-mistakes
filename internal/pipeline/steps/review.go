...
%s`,
	...
	historySection := executionContextPromptSection() + fixRoundProvenanceClause(sctx) + userIntentPromptSection(sctx) + intentConformanceReviewClause(sctx) + pipelineDeliveryPhaseClause() + testguidance.Rule + testguidance.ReviewerAction
	if !reviewFleetEnabled(sctx) {
