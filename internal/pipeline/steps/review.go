...
func (s *ReviewStep) Execute(sctx *pipeline.StepContext) (*pipeline.StepOutcome, error) {
	...
	if sctx.Fixing {
		...
		historySection := executionContextPromptSection() + toolBatchingPromptSection() + roundHistoryPromptSection(sctx) + userIntentPromptSection(sctx)
...
%s`,
	...
	historySection := executionContextPromptSection() + toolBatchingPromptSection() + roundHistoryPromptSection(sctx) + userIntentPromptSection(sctx) + intentConformanceReviewClause(sctx) + pipelineDeliveryPhaseClause()
