...
- A tool that is not on PATH and has no repository-local path must not be searched for or installed. If the scenario can be driven live through another route you already have, use it; otherwise report the affected scenario as "untested" with that reason instead of searching the machine for the tool.
