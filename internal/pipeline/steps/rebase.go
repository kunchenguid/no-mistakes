...
func (s *RebaseStep) Execute(sctx *pipeline.StepContext) (*pipeline.StepOutcome, error) {
	...
	// the contributor's local PR base branch but were never pushed to the
...
// branch carries commits that exist on the contributor's local PR base branch
// but were never pushed to its origin ref. In multi-session / monorepo setups
// the local base branch can carry another workstream's unpushed work; branching
// a fix off that local tip silently drags it into the PR when the feature branch
// is rebased onto the remote base. Returns nil when no such
...
// tip from the working repo, confirms that tip is ahead of the upstream ref and
...
// When the branch diff against the intended PR base is empty, SkipRemaining is set.
