...
// TestDaemonRestartMigratesAPreBindingGateHook: an install that predates the
// binding carries hooks without it, so the fix only reaches existing users if
// the daemon's gate migration regenerates them. Downgrade a live gate's managed
// hooks to the pre-binding shape, restart the daemon, and require a push whose
// shell exports a foreign NM_HOME to reach the owning daemon again.
func TestDaemonRestartMigratesAPreBindingGateHook(t *testing.T) {
	h := NewHarness(t, SetupOpts{Agent: "claude"})
	if out, err := h.Run("init"); err != nil {
		t.Fatalf("init: %v\n%s", err, out)
	}
	...
	for _, hook := range []string{"pre-receive", "post-receive"} {
		path := filepath.Join(gate, "hooks", hook)
		src, err := os.ReadFile(path)
		if err != nil {
			t.Fatalf("read %s: %v", hook, err)
		}
		downgraded := stripGateHomeBinding(string(src))
		if downgraded == string(src) {
			t.Fatalf("managed %s carries no home binding to remove", hook)
		}
		if err := os.WriteFile(path, []byte(downgraded), 0o755); err != nil {
			t.Fatalf("downgrade %s: %v", hook, err)
		}
	}
	...
	branch := "feature/migrated-hook-push"
	h.CommitChange(branch, "migrated.txt", "migrated\n", "add migrated file")
	...
	out, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("push after gate migration was rejected: %v\n%s", err, out)
	}
	t.Logf("push after migration:\n%s", out)
	run := h.WaitForRun(branch, 90*time.Second)
	t.Logf("owning daemon run after migration: id=%s status=%s", run.ID, run.Status)
}

// stripGateHomeBinding removes the NM_HOME derivation block from a rendered
// managed hook, reconstructing what an install from before the fix carried.
func stripGateHomeBinding(script string) string {
