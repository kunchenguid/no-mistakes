...
func TestDLOCK31ExecutableRetainedRefusal(t *testing.T) {
	for _, variant := range []string{"malformed-binding", "unbound", "dirty", "moved-anchor", "moved-head", "divergent", "policy-changed"} {
		t.Run(variant, func(t *testing.T) {
			...
			if beforeEdits == 0 {
				t.Fatal("provider edit counter did not observe original failure")
			}
			...
			if len(h.AgentInvocations()) != beforeCalls || dlock31ProviderEdits(t, provider) != beforeEdits {
				t.Fatal("refusal invoked a fixer or attempted publication")
			}
			if h.UpstreamBranchSHA(binding.Branch) != binding.RecordedHead || dlock31Git(t, h, workdir, "rev-parse", "HEAD") != beforeHead {
				t.Fatal("refusal changed upstream or retained head")
			}
			if dlock31Git(t, h, workdir, "status", "--porcelain") != beforeStatus {
				t.Fatal("refusal altered owned worktree")
			}
		})
	}
}
...
func TestDLOCK31ExecutableAlreadyPublishedStaleMetadata(t *testing.T) {
	...
	if err := database.ClearRetainedCIRepair(parked.ID, binding.RetainedHead); err != nil {
		t.Fatal(err)
	}
	...
	if stored.HeadSHA != binding.RecordedHead || h.UpstreamBranchSHA(binding.Branch) != binding.RetainedHead {
		t.Fatal("did not establish published-head/stale-metadata precondition")
	}
	beforeCalls := len(h.AgentInvocations())
	respondDLOCK31Fix(t, h, parked)
	...
	if len(h.AgentInvocations()) != beforeCalls+1 {
		t.Fatal("ordinary path did not invoke exactly one new fixer")
	}
	dlock31Git(t, h, workdir, "merge-base", "--is-ancestor", binding.RetainedHead, head)
	if got := dlock31Git(t, h, h.UpstreamDir, "show", head+":repair.txt"); got != "ordinary repair" {
		t.Fatalf("published content=%q", got)
	}
	if pending, err := database.RetainedCIRepair(parked.ID); err != nil || pending != nil {
		t.Fatalf("ordinary publication left binding: %+v %v", pending, err)
	}
}
...
func dlock31Git(t *testing.T, h *Harness, dir string, args ...string) string {
	...
	if err != nil {
		t.Fatalf("owned git %v: %v %s", args, err, out)
	}
...
func dlock31Workdir(t *testing.T, database *db.DB, runID string) string {
	...
	if err != nil || run == nil || run.WorktreePath() == "" {
		t.Fatalf("missing owned worktree: %+v %v", run, err)
	}
