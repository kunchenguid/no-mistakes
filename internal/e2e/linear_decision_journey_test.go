 ...
 content := `actions:
  ...
  - text: "no issues found"
    structured:
      ...
      body: "## Summary\nCorrected feature"
`
 if err := os.WriteFile(scenario, []byte(content), 0o644); err != nil { t.Fatal(err) }
 ...
 if out, err := h.RunInDir(initDir, "init"); err != nil { t.Fatalf("init: %v\n%s", err, out) }
 ...
 if err != nil || !strings.Contains(gate, "fix-feature") { t.Fatalf("initial review gate: %v\n%s", err, gate) }
 result, err := h.RunInDir(worktree, "axi", "respond", "--action", "fix", "--findings", "fix-feature")
 if err != nil { t.Fatalf("selected fix: %v\n%s", err, result) }
 run := h.WaitForRun(branch, 90*time.Second)
 if run.Status != types.RunCompleted { t.Fatalf("run status %s, error %v, response %s", run.Status, run.Error, result) }
 invocations := h.AgentInvocations()
 var reviewCount int
 var testDecision, documentDecision bool
 for _, inv := range invocations {
  if strings.Contains(inv.Prompt, "Review the code changes and return structured findings") { reviewCount++ }
  if strings.Contains(inv.Prompt, "You are validating a code change by driving the product itself") && strings.Contains(inv.Prompt, "Correct the feature value") { testDecision = true }
  if strings.Contains(inv.Prompt, "Find what this change made stale") && strings.Contains(inv.Prompt, "Correct the feature value") { documentDecision = true }
 }
 if reviewCount != 2 { t.Errorf("review turns = %d, want initial and fix rereview only", reviewCount) }
 if !testDecision || !documentDecision { t.Errorf("decision reached subsequent steps: test=%v document=%v", testDecision, documentDecision) }
 if got := h.UpstreamBranchSHA(branch); got != run.HeadSHA { t.Errorf("published head %s, want %s", got, run.HeadSHA) }
 for _, stepName := range []types.StepName{types.StepReview, types.StepTest, types.StepDocument, types.StepLint, types.StepPush} {
  ...
  if !ok || step.Status != types.StepStatusCompleted { t.Errorf("step %s not completed: %+v", stepName, step) }
 }
