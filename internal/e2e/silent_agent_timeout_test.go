...
func TestSilentAgentTimeoutReportsMeasuredEvidence(t *testing.T) {
	...
	// The hard limit that expired must be named...
	if !strings.Contains(surfaces, "reached its absolute wall-clock limit after 3s") {
		t.Fatalf("axi surfaces did not name the expired wall-clock limit:\n--- run ---\n%s\n--- status ---\n%s", runOut, statusOut)
	}
	// ...and the no-output duration must be a measurement, not the limit restated.
