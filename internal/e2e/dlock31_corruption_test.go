...
func mutateDLOCK31Repair(t *testing.T, h *Harness, database *db.DB, binding *db.RetainedCIRepair, variant string) string {
	...
	case "malformed-binding":
		...
		if err != nil { t.Fatal(err) }
		if count, err := result.RowsAffected(); err != nil || count != 1 { t.Fatalf("corrupted rows=%d %v", count, err) }
	case "unbound":
		if err := database.ClearRetainedCIRepair(binding.RunID, binding.RetainedHead); err != nil { t.Fatal(err) }
	...
	case "divergent", "moved-head":
		if variant == "divergent" { dlock31Git(t, h, workdir, "checkout", "--detach", binding.RecordedHead) }
...
func changeDLOCK31TrustedPolicy(t *testing.T, h *Harness, branch string) {
	...
	if out, err := h.Run("daemon", "restart", "--force"); err != nil { t.Fatalf("owned policy reload: %v %s", err, out) }
	if daemonPIDForRoot(t, h) == previous { t.Fatal("policy reload did not restart the owned daemon") }
}
