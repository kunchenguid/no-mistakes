...
.PHONY: build dist install test test-all test-tagged test-unit lint fmt clean docs docs-build docs-preview
...
test:
	go test -tags unit,integration -race ./...

test-all:
	go test -tags unit,integration,e2e -race ./...
