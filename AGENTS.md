...
- The paired `sync --recover --run <FULL-RUN-ID> --adopt-forward-head <FULL-COMMIT-ID>` path is exceptional operator authorization for one completed local-only historical incident shape, never an inferred `next_action` or generic descendant adoption. It runs inside the daemon's shared repo+branch lock, requires the latest exact run and complete successful Intent-through-Lint/skipped-publication step topology, preserves the exact candidate and immutable audit tuple, changes the recorded head by CAS, strict-fast-forwards the submitted local head, and stamps custody only after final full-predicate proof. `RecoverAuthorizedForwardHead` owns the contract; regressions live in `forward_recovery*_test.go`, `head_custody_test.go`, `sync_test.go`, and e2e `TestAxiExactForwardHeadRecoveryJourney`.
...
- Live Rebase and Review/Test/Document/Lint head transitions share the daemon's repo+branch lock with run admission and exceptional recovery. The candidate is immutably anchored and its exact old-to-candidate journal is prepared before an expected-old gate-ref CAS and matching active-run database CAS; agent-authored transitions are strict-forward, while Rebase alone may record an exact rewrite. Startup reconciles only a prepared gate-moved candidate before generic stale-run recovery and otherwise fails closed. Owners: `adoptPipelineHeadTransition`, `reconcilePreparedHeadAdvances`, and `coordinateRunAdmission`; regressions: `common_fix_adoption_test.go`, `head_advance_recovery_test.go`, and the branchsync forward-recovery matrix.
