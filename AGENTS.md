...
- The daemon runs `commands.*` from `.no-mistakes.yaml` verbatim via `sh -c`; resolve them from the run's exact immutable submitted commit through `loadSubmittedRepoConfig`, never from a mutable worktree file, another clone, or the remote default branch. `submitted_head_sha` is the recovery snapshot; legacy rows without it fall back to their retained `head_sha`. A candidate tree without `.no-mistakes.yaml` has empty commands and never borrows them from elsewhere.
- `agent` selects which process launches with the maintainer's credentials and remains loaded from the trusted default branch at a **pinned SHA** resolved by a fresh fetch. `document.instructions` and `disable_project_settings` are also trusted-only. The run aborts when those trusted boundaries cannot be read; when the opt-out is enabled, only adapters with verified effective suppression may launch. Non-executing fields (`ignore_patterns`, `auto_fix`, `commit`, `intent`, `test`) come from the submitted candidate snapshot.
- `allow_repo_commands` is the compatibility key for candidate `agent` selection. It is per-repo, read only from the trusted default-branch copy, and defaults `false`; candidate commands no longer depend on it. The e2e harness defaults it true through `SetupOpts.AllowRepoCommands` for legacy agent-selection behavior.
- Regressions: `TestLoadSubmittedRepoConfigUsesImmutableSubmittedHead`, `TestLoadSubmittedRepoConfigAbsentCandidateDoesNotBorrowLaterConfig`, `TestEffectiveRepoConfig_CandidateCommandsAndTrustedAgent`, `TestLoadTrustedRepoConfig_FailClosedOnFetchFailure`, `TestEffectiveRepoConfig_DocumentPolicyTrustedOnly`, `TestEffectiveRepoConfig_DisableProjectSettingsTrustedOnly`, `TestAssertGateTrustedConfigReadable_*`, `TestNewPipelineAgent_OptOut_*`, `TestLoadRecoveredConfig_BoundsFetchAndFailsClosed`, e2e `TestRunCommandsComeFromCandidateRepository` and `TestRepoConfigCandidateCommandsAndTrustedAgent`.
