...
- Stopping is only complete when the daemon **process** is gone, because the kernel releases the lock at process exit and nothing earlier. Losing IPC health does not prove exit: closing the IPC listener unlinks the socket at the *start* of shutdown, so a health-only stop returned while the old daemon still held the lock and `daemon restart`'s replacement child lost `acquireSingletonLock` and exited before readiness. `waitForDaemonStop` therefore also waits on a `daemonInstance` captured *before* the shutdown request (the daemon deletes its own PID file while tearing down), and `stopDetachedDaemon` closes its IPC client right after the shutdown call - the daemon drains in-flight connection handlers before exiting, so a stopper holding its client open deadlocks against the exit it is waiting for. Regressions: e2e `TestDaemonStopLeavesNoDaemonProcessOwningTheRoot`, `TestDaemonRestartReplacesTheDaemonWithExactlyOneOwner`.
