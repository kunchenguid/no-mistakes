...
- Reap a run by id from outside its worktree with `no-mistakes axi abort --run <id>` (`runAxiAbortByRunID`). It needs only `NM_HOME` + the daemon, not a repo/branch/worktree, because `ipc.MethodCancelRun` → `RunManager.HandleCancel` only cancels runs live in daemon memory. An unknown/inactive id, or a confirmed-stopped daemon, is an idempotent no-op (`aborted: false`), not an error. This is how an orphaned monitor (worktree torn down before merge) gets reaped deterministically. Bare `axi abort` (no `--run`) stays worktree/branch-scoped. If the daemon's liveness can't be confirmed - e.g. the process is alive but its socket is stuck and the health check hits the bounded `daemon_connect_timeout` - that ambiguous state is surfaced as an error instead of a false-negative no-op, since a still-executing run would otherwise never get reaped. Regression: `TestAxiAbortByRunIDSurfacesConnectTimeoutInsteadOfNoOp`.
