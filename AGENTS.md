...
- On Windows an npm-installed agent CLI (`claude`, `codex`, `copilot`, `pi`, `acpx`, plus the `opencode`/`rovodev` servers) is a `.cmd`/`.bat` shim that Go's `exec` would run through `cmd.exe`, whose `%*` forwarding corrupts a multi-line `-p` prompt and, in the console-less daemon, never delivers it to the wrapped `.exe`, so the CLI starts interactive and emits no result (issue #427).
  ...
  It is best-effort: a non-npm install or an unrecognizable shim falls back to the original binary, and the regex is anchored to the trailing `%*` so npm's `IF EXIST "%dp0%\node.exe"` node-launcher guard (no `%*` after its quoted `.exe`) is not misresolved.
...
**Cygwin/MSYS Git Argument Globbing (`internal/git/env.go`)**

- On Windows a native process can hand a child only a single command-line string, and a Cygwin- or MSYS2-linked `git` re-parses and globs it at startup, stripping the braces from an argument like `refs/heads/main^{commit}` (turning it into `refs/heads/main^commit`), which git then rejects as ambiguous (issue #427). The failure appears only when the native daemon spawns git; from a Cygwin shell the braces survive because argv passes through Cygwin's own exec. `NonInteractiveEnv` appends `noglob` to the `CYGWIN` and `MSYS` env vars (via `disableChildArgGlobbing`) to disable that globbing. We always pass git explicit, already-resolved arguments and never rely on runtime globbing, so this is safe. Existing user options are preserved (`noglob` is appended only when absent), and the keys are matched case-insensitively (`EqualFold`) so an ambient mixed-case `Cygwin=winsymlinks:native` is extended rather than shadowed by a fresh uppercase `CYGWIN=noglob` under os/exec's own case-insensitive last-wins dedup. No-op off Windows. Regressions: `TestNonInteractiveEnv_DisablesCygwinGlobbing`, `TestNonInteractiveEnv_PreservesCygwinOptions`, `TestDisableChildArgGlobbing_PreservesMixedCaseCygwin` (`internal/git/env_test.go`).
