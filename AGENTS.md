...
**Cygwin/MSYS Git Argument Globbing (`internal/git/git.go`)**

- On Windows a native process can hand a child only a single command-line string, and a Cygwin- or MSYS2-linked `git` re-parses and globs it at startup, stripping the braces from an argument like `refs/heads/main^{commit}` (turning it into `refs/heads/main^commit`), which git then rejects as ambiguous (issue #427). The failure appears only when the native daemon spawns git; from a Cygwin shell the braces survive because argv passes through Cygwin's own exec. Rather than mutate the child environment, peel commits with `^0`, the brace-free equivalent of `^{commit}`: it dereferences an annotated tag to its commit (recursively) and errors on a non-commit, but contains no glob metacharacters, so a Cygwin/MSYS2 git leaves it intact. `ResolveRef` and `RefExists` build their rev-parse argument as `ref+"^0"`. `^{commit}` was the only brace-bearing git argument in the tree. No behavior change off Windows: the peel is identical on every platform. Regressions: `TestResolveRef`, `TestResolveRef_MissingRef`, `TestRefExists` (`internal/git/git_showfile_test.go`).
