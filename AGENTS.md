...
- GitHub readiness is the union of the PR check rollup and every Actions workflow run returned by the Actions API for the exact pipeline head SHA. A workflow rejected before creating jobs/check-runs is absent from `gh pr checks` but still present in that API; run discovery errors and unknown run states fail closed instead of certifying a green rollup. Regressions: `TestGetChecksIncludesFailedWorkflowRunMissingFromPRRollup`, `TestCIStep_FailedHeadWorkflowRunPreventsChecksPassed`.
