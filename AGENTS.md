...
**Gate-Instruction Neutralization (`internal/agent`, `internal/daemon`)**

- Under `disable_project_settings`, the gate agent must neutralize the target checkout's project agent-instruction files (AGENTS.md/CLAUDE.md) and every project-controlled context source so a repo that is itself an agent-orchestration harness cannot install a fleet-captain identity on the reviewer. `EnsureGateNeutralized` (`internal/agent/agent.go`) fails closed for any adapter whose `NeutralizesGateInstructions()` reports false.
- Verified harnesses: **codex** (`-c project_doc_max_bytes=0` + `--ignore-rules`), **claude** (`--setting-sources user`), and **opencode** (`--no-project-instructions` + `--pure`). The codex/claude knobs are honest about the EFFECTIVE value: an operator override that re-enables the project surface (`project_doc_max_bytes>0`, `--setting-sources ...,project`) makes `NeutralizesGateInstructions()` return false so the gate refuses. opencode's managed flags are appended LAST in `buildOpencodeServeArgs` so yargs last-wins enforces them; there is no operator flag that re-enables project instructions once `--no-project-instructions` is in effect, so `NeutralizesGateInstructions()` reports `disableProjectSettings` directly.
- opencode requires a binary that supports the `--no-project-instructions` serve flag (paired OpenCode change). `probeOpencodeNoProjectInstructions` (`internal/agent/opencode_http.go`) scans `opencode serve --help` for the flag BEFORE starting the server; an older binary that omits the flag stops with a concrete diagnostic naming the capability and upgrade path, and the serve process itself exits non-zero on the unknown flag as defense-in-depth. The env-var analogue `OPENCODE_DISABLE_PROJECT_INSTRUCTIONS` is intentionally NOT used: an older binary would silently ignore an unknown env var, which is the failure mode the CLI switch prevents.
- Operator model args (e.g. `--model ollama-cloud/glm-5.2`) from `agent_args_override.opencode` are preserved because they precede the managed neutralization flags and do not conflict.
- Regressions: `internal/agent/gateneutralize_test.go`, `internal/agent/opencode_neutralize_test.go`, `internal/agent/opencode_neutralize_unix_test.go`, `internal/agent/opencode_adversarial_test.go`, `internal/agent/servearg_test.go` (`TestBuildOpencodeServeArgs_OptOut*`), `internal/daemon/gateneutralize_test.go`.
