...
- Intent carries provenance: an explicit `axi run --intent` persists `Source==db.RunIntentSourceAgent` ("agent", score 1); a transcript match persists the agent name ("claude"/"codex"/...). The executor propagates it as `StepContext.IntentSource` alongside `UserIntent` (`executor.go`); dropping it was the provenance-erasure bug that demoted an authoritative intent to an ignorable hint.
...
- The deterministic net-deleted-author-lines git-diff backstop (option C) is intentionally NOT built - see the `TODO(intent-conformance-C, HELD)` pointer in `review.go` and addendum `data/nm-conformance-lowoverhead-addendum-w4/report.md` §5.1/§5.3.
