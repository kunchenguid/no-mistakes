...
- GitHub readiness is the union of the PR check rollup and every Actions workflow run for the exact pipeline head SHA. A workflow rejected before creating jobs/check-runs is absent from `gh pr checks` but present in `gh run list --commit`; run discovery errors and unknown run states fail closed instead of certifying a green rollup. Regressions: `TestGetChecksIncludesFailedWorkflowRunMissingFromPRRollup`, `TestCIStep_FailedHeadWorkflowRunPreventsChecksPassed`.
