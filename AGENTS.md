...
- Review auto-fix is disabled by default (`config.go` `autoFixDefaults` `Review: 0`), so with default config every actionable review finding parks for an agent decision. A repo or global `auto_fix.review > 0` override re-enables review auto-fix through `AutoFixLimit(types.StepReview)` and the executor auto-fix loop, so keep the skill, live `axi` note, and docs qualified if you touch review auto-fix.
