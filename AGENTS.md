...
**Provider CI Authority (`scm.Capabilities.RequiresChecks`)**

- An empty check list from `Host.GetChecks` is ambiguous: on GitHub it usually means "no CI configured" (safe to treat as ready), but on Azure DevOps it routinely means the PR is simply not validated yet (no build-validation policy, not yet queued, or path-scoped `notApplicable`). `scm.Capabilities.RequiresChecks` distinguishes them: when true (ADO only; GitHub/GitLab/Bitbucket leave it false), the CI step (`internal/pipeline/steps/ci.go`) routes an empty check list after the grace period to a fail-safe `NeedsApproval` park (`ciNoChecksBlockedOutcome`) instead of `cimonitor.NoChecksPassedMsg`/ready. This keeps `cimonitor.ChecksPassed` honest without changing the constant's meaning for GitHub. Follow-up B3 (deferred): cross-check `az repos policy list` to synthesize a pending check for a required-but-not-yet-queued build. `azStatusBucket` (`internal/scm/azuredevops/parse.go`) maps `notApplicable`/empty to dropped but any unrecognized NON-empty status to pending, so an unexpected status can never manufacture a false-empty list (B2).
- **ADO content-vs-human gate classification (`policyEval.isCICheck`, `internal/scm/azuredevops/parse.go`).** ADO returns human review / sign-off / attestation gates in the same `az repos pr policy list` payload as content builds, and they report a blocking `rejected` on any normal open PR that is merely awaiting human action. no-mistakes gates only on content-influenced automation (build/test/e2e build validation, code coverage, Component Governance security+license status checks); it must ignore the human gates even when rejected, or the CI auto-fix loop chases a signal no code change can satisfy. Classify by **stable policy identity, never the localizable `settings.displayName`**: own-type human gates (Minimum/Required reviewers, Comment requirements, Require a merge strategy, Work item linking, **Ownership Enforcer**, **Proof Of Presence**) match `humanPolicyTypes` on the type displayName (a stable English policy-type name, unlike the user-facing settings string); Code Review Compliance Policy arrives through the generic **`Status` policy type** (so a Build/Status allow-list alone lets it leak) and is matched by its stable `(statusGenre, statusName)` = (`microsoft-policy-service`, `CodeReviewCompliancePolicy`) via `humanStatusGate`. Ownership Enforcer and Proof Of Presence are their OWN policy types, NOT Status checks (confirmed live on ADO PR 16330529) - naming them in `humanPolicyTypes` makes the exclusion explicit rather than accidental. Every other Build/Status policy stays a content check (fail-safe: an unrecognized gate is surfaced, never silently dropped). Regressions: `TestIsCICheck`, `TestGetChecksExcludesHumanGatesByIdentity`, `TestGetChecksGatesOnContentStatusChecks` (`internal/scm/azuredevops/azuredevops_test.go`).
