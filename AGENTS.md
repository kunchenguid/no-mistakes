...
- Agent memory files (`AGENTS.md`, `CLAUDE.md`) are hands-off for every step prompt (`agent.MemoryFilesRule`); the document step may only correct factually wrong content, and the conflict resolvers use `agent.MemoryFilesConflictRule`. Do not reintroduce exhaustive doc-sweep language into the document prompt. Owner: `documentation-guidance` skill.
