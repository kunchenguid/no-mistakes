...
- **Rebase base comes from the freshly-fetched authoritative remote, never local/stale state.** The rebase step (`internal/pipeline/steps/rebase.go`) fetches `origin/<default>` and `origin/<branch>` (or the fork tracking ref) and rebases onto those remote-tracking refs - never the local default branch. The push-branch tracking ref is refreshed **even on a force push** (the force push only skips *rebasing onto* it, not recording it) so the push step has an accurate lease anchor.
...
- **Every force-push is lease-guarded against discarding unseen upstream commits.** All force-push sites (`PushStep` in `push.go`, CI auto-fix `pushUpdatedHeadSHA` in `ci_fix.go`) route through `resolveForcePushDecision` (`internal/pipeline/steps/forcepush.go`). It re-reads the live remote head and allows the push only when: the branch is new; the remote already equals the head; the remote still equals `lastSeenSHA` (what the run last observed); or every commit now on the remote is already incorporated **by patch-id** (`git rev-list --cherry-pick --right-only`) into the head being pushed. Otherwise it returns `forcePushWouldDiscardError` and the caller must NOT push.
- **`lastSeenSHA` differs by caller and must stay correct:** the push step passes the freshly-fetched remote-tracking ref (`lastFetchedBranchTip`, the exact commit the branch was rebased against); the CI step passes `Run.HeadSHA` (the run's last-recorded/pushed head). Anchoring the lease to a SHA read from the remote *immediately before pushing* is the original #281 bug - it always passes and protects nothing. Never reintroduce that; never degrade to a bare `--force`/`--force-with-lease` without an explicit anchor when the ls-remote/fetch fails (fail closed instead). Regressions: `TestCIStep_CommitAndPush_RefusesToClobberUnseenUpstreamCommit` (#281), `TestPushStep_RefusesToClobberAdvancedUpstreamBranch` (#305), and `TestResolveForcePushDecision_*`.
