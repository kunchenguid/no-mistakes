...
- Pre-provenance (v1.37) terminal rows with NULL `submitted_head_sha` and NULL push binding are recovered by the same guarded path, not left as `blocked_recover_not_applicable`: `isLegacyStranded`/`legacyStrandedRun` select such a row only when no modern run matched the branch (so real provenance and active runs still win), inspect classifies it identically (`blocked_pipeline_owned_recoverable`), and Recover reuses the whole decision matrix — custody is never inferred from the mutable branch/`head_sha`; the gate branch must still equal the recorded terminal head. `submitted_head_sha` is always set by `InsertRun`, so this can never fire for a modern run.
- Public guidance is owned by `internal/skill/skill.go` plus live AXI strings, then regenerated with `make skill`. Core regressions live in `internal/branchsync` (incl. `recover_test.go`, `TestRecoverLegacy*`), `internal/cli/sync_test.go`, `internal/tui/branch_sync_test.go`, and e2e `TestAxiBranchSyncJourney` / `TestAxiCustodyRecoveryJourney`.
