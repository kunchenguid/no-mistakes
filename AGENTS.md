...
- On Windows an npm-installed agent CLI (`claude`, `codex`, `copilot`, `pi`, `acpx`, plus the `opencode`/`rovodev` servers) is a `.cmd`/`.bat` shim that Go's `exec` would run through `cmd.exe`, whose `%*` forwarding corrupts a multi-line `-p` prompt and, in the console-less daemon, never delivers it to the wrapped `.exe`, so the CLI starts interactive and emits no result (issue #427).
  Resolve the binary with `resolveAgentBinary` (`internal/agent/shim_windows.go`, no-op stub in `shim_other.go`) before building the `*exec.Cmd`, so exec launches the native `.exe` directly with a proper argv array.
  It is best-effort and scoped to npm: the wrapped `.exe` is launched directly only when it resolves to a path under `node_modules` (`underNodeModules`), so a hand-written `.cmd` wrapper that runs its own setup before launching an unrelated `.exe` is left untouched. A non-npm install or an unrecognizable shim falls back to the original binary, and the regex is anchored to the trailing `%*` so npm's `IF EXIST "%dp0%\node.exe"` node-launcher guard (no `%*` after its quoted `.exe`) is not misresolved.
  Every agent `runOnce` and the shared `startServerWithPort` already call it; apply it to any new agent spawn site.
  Regressions: `TestResolveAgentBinary_ResolvesShimToExe`, `TestShimNativeTarget_NpmClaudeShim`, `TestShimNativeTarget_NodeLauncherFallsBack` (`internal/agent/shim_windows_test.go`).
...
**Cygwin/MSYS Git Argument Globbing (`internal/git/git.go`, `internal/git/env.go`)**

- On Windows a native process can hand a child only a single command-line string, and a Cygwin- or MSYS2-linked `git` re-parses and globs it at startup, stripping the braces from an argument like `refs/heads/main^{commit}` (turning it into `refs/heads/main^commit`, which git rejects as ambiguous) and expanding any bare `*`, `?`, or `[...]` (issue #427). The failure appears only when the native daemon spawns git; from a Cygwin shell the braces survive because argv passes through Cygwin's own exec.
- Fix: the git spawn path scopes `CYGWIN`/`MSYS=noglob` to the git subprocess only. `git.Run` and `RefExists` (`internal/git/git.go`) set `cmd.Env = gitSpawnEnv(dir)`, which wraps `NonInteractiveEnv` with `disableChildArgGlobbing` (`internal/git/env.go`); it appends `noglob` case-insensitively, preserving any existing options (e.g. an ambient `Cygwin=winsymlinks:native`). This is deliberately NOT applied inside `NonInteractiveEnv`, because that env is the base for the coding-agent env (`agent.gitSafeEnv`); injecting there disabled globbing for every Cygwin/MSYS2 tool the agents exec (a real blast radius, reverted once in commit `d36bcd9`). Because it is scoped to `git.Run`/`RefExists`, every no-mistakes-owned git call is protected and can use plain `^{commit}`/`^{tree}` peels with no per-argument workaround. An earlier iteration of this branch instead avoided braces per call site (`^0` for commits, `git log --format=%T` for the one tree read); noglob makes that whack-a-mole unnecessary, so those were reverted to the plain brace forms.
- One gap is accepted: agents that shell out to git through `gitSafeEnv` stay glob-enabled, so a `^{...}` argument they build is still exposed. It is a documented follow-up, not closable here (we cannot noglob agent-git without noglob-ing every other Cygwin tool the agent runs); the realistic exposure (`@{u}`, `HEAD@{1}`) is uncommon and non-fatal.
- No behavior change off Windows: `disableChildArgGlobbing` is a no-op there. Regressions: `TestGitSpawnEnv_DisablesCygwinGlobbing`, `TestNonInteractiveEnv_KeepsGlobbing` (`internal/git/env_test.go`), `TestGitSafeEnv_KeepsCygwinGlobbing` (`internal/agent/env_test.go`), plus `TestResolveRef`, `TestRefExists` (`internal/git/git_showfile_test.go`).
...
- On Windows the embedded no-mistakes executable path must be forward-slashed before it goes into the POSIX hook script: `os.Executable` returns a backslash path (e.g. `C:\Users\me\...\no-mistakes.exe`), and Cygwin's `sh` (which git may use to run the hook) consumes the backslashes as escapes, so the embedded path fails to exec with `command not found` (issue #427). `hookBinPath` converts `\` to `/` on Windows and is a no-op elsewhere; Git for Windows' bundled `sh` tolerates the forward-slash form too, so it is safe for both shells. Regression: `TestHookBinPath` (`internal/git/hook_test.go`).
