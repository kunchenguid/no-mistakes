...
- `document.instructions` (the repo's documentation placement policy) and `disable_project_settings` (the gate-agent behavior-resource isolation opt-out) are also trusted-only: a pushed branch must not weaken either boundary. When the opt-out is enabled, only adapters with verified effective isolation may launch. Non-executing fields (`ignore_patterns`, `auto_fix`, `commit`, `intent`, `test`) are still read from the pushed branch.
