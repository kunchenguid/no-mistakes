...
- The check itself is the shared composite action in [`.github/actions/require-no-mistakes`](.github/actions/require-no-mistakes/README.md). Other repositories enforce the identical gate by calling that action pinned to a release tag or commit SHA, rather than copying the enforcement shell into their own workflow.
